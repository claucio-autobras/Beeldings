...
// ─── MQTT — diagnóstico da sessão no broker ──────────────────────────────────
...
/** Consulta a sessão do usuário dedicado sem expor a senha. */
export async function getMqttConnectionStatus(deviceId: string): Promise<MqttConnectionStatus> {
  return apiGet<MqttConnectionStatus>(`/devices/${deviceId}/mqtt-connection`);
}
...
export type PointSiteMetric = 'energy' | 'water' | 'runtime' | null;
export type PointSiteMetricMode = 'cumulative' | 'rate' | 'state' | null;
...
/** Define ou remove a fonte semântica usada na visão geral do site. */
export async function setPointSiteMetric(
  deviceId: string,
  pointId: string,
  ...
  await apiPatch(`/devices/${deviceId}/points/${pointId}`, {
    siteMetric,
    siteMetricMode,
    ...(siteMetric ? { unit } : {}),
  });
