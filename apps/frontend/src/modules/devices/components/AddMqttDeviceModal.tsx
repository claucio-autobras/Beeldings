...
export default function AddMqttDeviceModal({ open, onClose, onCreated }: Props) {
  ...
  /**
   * Aceita tanto o sub-tópico recomendado pela tela quanto um tópico completo
   * copiado do manual. Evita gerar raiz/raiz/... quando o operador cola o
   * caminho completo no campo relativo.
   */
  const scopedTopic = (value: string): string => {
    const raw = value.trim().replace(/^\/+/, '');
    if (!raw || !effectivePrefix) return raw;
    return raw === effectivePrefix.slice(0, -1) || raw.startsWith(effectivePrefix)
      ? raw
      : `${effectivePrefix}${raw}`;
  };
    ...
    try {
      const result = await sampleMqttTopic({
        ...
        gatewayId: siteGatewayId,
          topic: scopedTopic(sampleTopicInput),
    ...
    try {
      ...
      const device = await createMqttDeviceWithPoints({
        ...
        ...(isRootMode ? { rootTopic: rootNorm } : {}),
         heartbeatTopic: heartbeatSub.trim() ? scopedTopic(heartbeatSub) : null,
        ...
        selectedPoints: validRows.map((r) => ({
          ...
          displayName: r.displayName.trim() || r.tag.trim(),
           sourceTopic: scopedTopic(r.sourceTopic),
          jsonPath:    r.jsonPath.trim(),
          valueType:   r.valueType,
          unit:        r.unit.trim(),
          write: r.writeEnabled && r.commandTopic.trim()
            ? {
                 commandTopic: scopedTopic(r.commandTopic),
                payloadTemplate: r.payloadTemplate.trim(),
                 responseTopic: r.responseTopic.trim() ? scopedTopic(r.responseTopic) : null,
              }
            : null,
        })),
      });
  ...
  if (createdDevice) {
    ...
    const broker = brokerDetails(cred?.broker);
    return (
          ...
          <div className="px-5 py-4 space-y-3">
            ...
            {cred ? (
              <div className="grid grid-cols-1 gap-1.5 text-xs border border-sky-200 dark:border-sky-900 rounded-lg p-3 bg-sky-50/50 dark:bg-sky-950/30">
                ...
                {broker && (
                  <>
                    <div><span className="text-muted-foreground">Host:</span> <span className="font-mono text-foreground">{broker.host}</span></div>
                    <div>
                      <span className="text-muted-foreground">Porta:</span>{' '}
                      <span className="font-mono text-foreground">{broker.port}</span>{' '}
                ...
                <div><span className="text-muted-foreground">Tópico raiz:</span> <span className="font-mono text-foreground break-all">{createdDevice.mqttConfig?.rootTopic}</span></div>
        ...
        <div className="flex-1 px-5 py-4 overflow-y-auto space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p className="text-[11px] text-muted-foreground mt-1">O gateway selecionado fará o bridge dos tópicos.</p>
            </div>
          </div>
          ...
          <div className="space-y-2 border border-border rounded-lg p-3 bg-muted/20">
            ...
            {isRootMode && (
              <div>
                ...
                <p className="text-[11px] text-muted-foreground mt-1">
                  Precisa ser único na plataforma. Os sub-tópicos abaixo podem ser informados de forma relativa
                  (ex.: <span className="font-mono">from/keepalive</span>) ou como caminho completo.
                  Para SmartPDU, o formato do raiz é <span className="font-mono">{'{facility}/pdu/{deviceID}'}</span>.
                </p>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1">
              <div>
                <label className={labelCls}>Tópico de presença (opcional)</label>
                <input className={`${inputCls} font-mono`} placeholder="ex: from/keepalive" value={heartbeatSub} onChange={(e) => setHeartbeatSub(e.target.value)} />
              </div>
              <div>
              ...
              <p className="sm:col-span-2 text-[11px] text-muted-foreground">
                  Informe o sub-tópico publicado pelo equipamento (também aceitamos o caminho completo).
