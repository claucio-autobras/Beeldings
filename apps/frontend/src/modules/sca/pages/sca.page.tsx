...
const HEALTH_METRICS = ['cpu', 'memory_available', 'ram_total', 'temperature', 'packet_loss'] as const;
...
const TEST_STATE_LABELS: Record<string, string> = {
  SUPPORTED: 'amostra válida',
  UNSUPPORTED: 'não suportado pelo firmware',
  TEMPORARY_ERROR: 'sem leitura (erro transitório)',
  NO_PERMISSION: 'sem permissão SNMP',
};
          ...
          {controllers.map((controller) => {
            ...
            const healthTiles = buildHealthTiles(
              ...
              (tag) => {
                ...
                return entry
                  ? { value: entry.value, unreliable: entry.unreliable, timestamp: entry.timestamp }
                  : null;
              },
            );
            ...
            <div className={[
              ...
              <p className="font-medium">
                {testResult.reachable
                  ...
                  : testResult.cause === 'community'
                    ? t('Agente SNMP respondeu com outra community')
                    : t('Controladora não respondeu ao SNMP (verifique community, porta e se o SNMP está habilitado)')}
              </p>
              <ul className="space-y-1 pt-1">
                {HEALTH_METRICS.map((m) => {
                  const canonicalKey = m === 'cpu'
                    ? 'cpu_usage'
                    : m === 'temperature'
                      ? 'cpu_temperature'
                      : m;
                  const result = testResult.metricResults?.[m]
                    ?? testResult.metricResults?.[canonicalKey];
                  const legacyValue = testResult.values[m];
                  const value = result?.value
                    ?? (typeof legacyValue === 'number' && Number.isFinite(legacyValue) ? legacyValue : null);
                  const state = result?.state
                    ?? (value !== null ? 'SUPPORTED' : 'TEMPORARY_ERROR');
                  const unit = result?.unit ?? (m === 'cpu' ? '%' : m === 'temperature' ? '°C' : '');
                  const formatted = value === null
                    ? TEST_STATE_LABELS[state] ?? t('sem leitura')
                    : formatHealthValue(m, value, unit) ?? String(value);
                  return (
                    <li key={m} className={[
                      ...
                      <div className="flex justify-between gap-3">
                        ...
                        <span className={state === 'SUPPORTED' ? 'font-mono text-emerald-700 dark:text-emerald-300' : 'font-medium'}>
                          {formatted}
                        </span>
                      </div>
                      <div className="flex justify-between gap-3 text-[10px] opacity-75">
                        <span>{TEST_STATE_LABELS[state] ?? state}</span>
                        <span className="font-mono">
                          {result?.selectedOid ?? testResult.oids[m] ?? t('fonte não selecionada')}
                          {result?.scale && result.scale !== 1 ? ` ×${result.scale}` : ''}
                        </span>
                      </div>
                    </li>
                  );
