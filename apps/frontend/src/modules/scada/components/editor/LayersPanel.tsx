...
import { Check, ChevronDown, ChevronUp, GripVertical, Layers, Lock, LockOpen, PackagePlus, Pin, X } from 'lucide-react';
import { useEditorStore } from '../../store/editor.store';
import { isPinnedNavWidget, type Widget, type WidgetType } from '../../types/scada.types';
...
const TYPE_LABELS: Partial<Record<WidgetType, string>> = {
  ...
  'manual-call-point': 'Acionador Manual', 'zone-module': 'Módulo de Zona', 'monitor-module': 'Módulo Monitor',
  'command-module': 'Módulo de Comando', 'flow-switch': 'Chave de Fluxo', 'fire-panel': 'Central de Alarme',
  'fire-siren': 'Sirene', 'heat-detector': 'Detector Térmico', sprinkler: 'Sprinkler',
  'fire-hydrant': 'Hidrante', 'fire-extinguisher': 'Extintor', 'fire-pump': 'Bomba de Incêndio',
  'fire-damper': 'Damper Corta-Fogo', 'fire-door': 'Porta Corta-Fogo',
  camera: 'Câmera CFTV', lighting: 'Iluminação',
  'component-instance': 'Componente fixo',
  line: 'Linha', rectangle: 'Retângulo', square: 'Quadrado', circle: 'Círculo',
  ellipse: 'Elipse', triangle: 'Triângulo', polygon: 'Polígono', 'titled-area': 'Área', separator: 'Separador',
...
function widgetLabel(w: Widget): string {
  if (w.editorLabel?.trim()) return w.editorLabel.trim();
  if (w.type === 'component-instance' && w.componentName) return w.componentName;
...
export function LayersPanel() {
  ...
  function applyMove(id: string, dir: -1 | 1) {
    const moving = screen?.widgets.find((w) => w.id === id);
    if (!moving || isPinnedNavWidget(moving)) return;
  ...
  function handleRowClick(e: React.MouseEvent, id: string, index: number) {
    const clicked = screen?.widgets.find((w) => w.id === id);
    // A barra fixa é apenas referência nesta tela; não entra em seleção nem
    // em reordenação. Para editá-la, abra a tela de origem indicada no badge.
    if (clicked && isPinnedNavWidget(clicked)) return;
          ...
          <ul className="px-2">
            {ordered.map((w, index) => {
              ...
              return (
                <li
                  key={w.id}
                  draggable={!isPinnedNavWidget(w)}
                  onDragStart={() => { if (!isPinnedNavWidget(w)) setDragId(w.id); }}
                  onDragEnd={reset}
                  onDragOver={(e) => {
                    e.preventDefault();
                ...
                >
                  ...
                  {isPinnedNavWidget(w) && <span title="Barra fixa — editar na tela de origem"><Pin className="h-3 w-3 shrink-0 text-slate-500" /></span>}
                  <button
                    type="button"
