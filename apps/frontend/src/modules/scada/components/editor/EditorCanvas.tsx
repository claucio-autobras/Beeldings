...
import {
  isPinnedNavWidget, clampPinnedToolbarH, clampPinnedSidebarW, normalizeNavToolbarLogo,
  normalizeNavToolbarLogoWidth, normalizeNavToolbarLogoHeight,
  type Widget, type WidgetType, type PipeWidget, type PipePoint, type PolygonWidget, type PolygonPoint,
  scaleScadaPolygonPoints,
} from '../../types/scada.types';
import { useEditorStore } from '../../store/editor.store';
import { hasScreenTelemetryBindings } from '../../types/scada.types';
...
/**
 * Handle visual da logo. É um botão separado da superfície da toolbar para
 * deixar claro que o gesto altera somente a logo, não a barra inteira.
 * O SVG é espelhado para que a diagonal acompanhe o canto usado por cada lado.
 */
function LogoResizeHandle({
  ...
  return (
    <button
      type="button"
      data-testid="nav-toolbar-logo-resize-handle"
      aria-label="Redimensionar logo"
      title="Redimensionar logo — arraste para ajustar largura e altura"
      onMouseDown={onMouseDown}
      className="group flex items-center justify-center rounded-sm border border-cyan-200 bg-slate-950/95 p-0 shadow-[0_0_0_1px_rgba(15,23,42,0.95),0_0_0_3px_rgba(34,211,238,0.28),0_2px_5px_rgba(2,6,23,0.8)] transition-colors hover:bg-cyan-950 focus-visible:outline focus-visible:outline-2 focus-visible:outline-white"
      style={{
        ...style,
        appearance: 'none',
        touchAction: 'none',
    ...
    >
      <span
        aria-hidden="true"
        className="flex items-center justify-center rounded-[2px] bg-white/95 transition-transform group-hover:scale-110"
        style={{ width: '62%', height: '62%' }}
      >
        <svg
          aria-hidden="true"
          viewBox="0 0 16 16"
          style={{ width: '100%', height: '100%', transform: position === 'right' ? 'scaleX(-1)' : undefined }}
        >
          <path
...
export function EditorCanvas() {
  const {
    ...
    pipeDraw, setPipeDraw, polygonDraw, setPolygonDraw,
  } = useEditorStore();
  ...
  const logoResizeRef = useRef<{
    ...
    startY: number;
    id: string;
    logo: NonNullable<ReturnType<typeof normalizeNavToolbarLogo>>;
  } | null>(null);
  ...
  const [polygonPts, setPolygonPts] = useState<PolygonPoint[]>([]);
  const [polygonCursor, setPolygonCursor] = useState<PolygonPoint | null>(null);
  const [selectedPolygonVertex, setSelectedPolygonVertex] = useState<{ widgetId: string; index: number } | null>(null);
  const selectedPolygonVertexRef = useRef<{ widgetId: string; index: number } | null>(null);

  function selectPolygonVertex(selection: { widgetId: string; index: number } | null) {
    selectedPolygonVertexRef.current = selection;
  ...
  const telemetryEnabled = Boolean(screen && hasScreenTelemetryBindings(screen.widgets));
  const { getValue: getTagValue, getReading: getTagReading, getPointStatus: getTagStatus } = useScreenTelemetry(devices, telemetryEnabled);
  ...
  useEffect(() => {
    function onKeyDown(e: KeyboardEvent) {
      ...
      if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'a' && !previewMode && !inField) {
        e.preventDefault();
        const store = useEditorStore.getState();
        store.setSelection((store.screen?.widgets ?? [])
          .filter((w) => !w.locked && !isPinnedNavWidget(w))
          .map((w) => w.id));
        return;
      }
      const store = useEditorStore.getState();
      const drawing = store.pipeDraw || store.polygonDraw;
      const vertexSelection = selectedPolygonVertexRef.current;
      const polygon = vertexSelection && !drawing && !inField
        ? store.screen?.widgets.find((widget) => widget.id === vertexSelection.widgetId)
        : null;
      if (
        ...
        !polygon.locked &&
        vertexSelection &&
        vertexSelection.index >= 0 &&
        vertexSelection.index < polygon.points.length
      ) {
        e.preventDefault();
        if (polygon.points.length > 3) {
          const abs = scaleScadaPolygonPoints(polygon.points, polygon.width, polygon.height, polygon.width, polygon.height)
            .map((point) => ({ x: point.x + polygon.x, y: point.y + polygon.y }));
          const next = abs.filter((_, index) => index !== vertexSelection.index);
          const minX = Math.min(...next.map((point) => point.x));
          const minY = Math.min(...next.map((point) => point.y));
          const maxX = Math.max(...next.map((point) => point.x));
          const maxY = Math.max(...next.map((point) => point.y));
          const nextIndex = Math.min(vertexSelection.index, next.length - 1);
          store.updateWidget(vertexSelection.widgetId, {
            x: Math.round(minX),
            y: Math.round(minY),
          ...
          store.commitHistory();
          selectPolygonVertex({ widgetId: vertexSelection.widgetId, index: nextIndex });
        }
        return;
      }
      if ((e.key === 'Delete' || e.key === 'Backspace') && !previewMode && !drawing) {
      ...
      if (!previewMode && !inField && !drawing && ['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(e.key)) {
        const currentVertex = selectedPolygonVertexRef.current;
        const selectedPolygon = currentVertex && store.screen?.widgets.find((widget) => widget.id === currentVertex.widgetId);
        if (
          currentVertex &&
          selectedPolygon?.type === 'polygon' &&
          !selectedPolygon.locked &&
          currentVertex.index >= 0 &&
          currentVertex.index < selectedPolygon.points.length
        ) {
          e.preventDefault();
          const step = e.shiftKey ? 10 : 1;
          const dx = e.key === 'ArrowLeft' ? -step : e.key === 'ArrowRight' ? step : 0;
          const dy = e.key === 'ArrowUp' ? -step : e.key === 'ArrowDown' ? step : 0;
          const abs = scaleScadaPolygonPoints(
            selectedPolygon.points,
            selectedPolygon.width,
            selectedPolygon.height,
            selectedPolygon.width,
            selectedPolygon.height,
          ).map((point) => ({ x: point.x + selectedPolygon.x, y: point.y + selectedPolygon.y }));
          const next = [...abs];
          const point = {
            x: abs[currentVertex.index].x + dx,
            y: abs[currentVertex.index].y + dy,
          };
          if (e.shiftKey) {
            point.x = Math.round(point.x / 10) * 10;
            point.y = Math.round(point.y / 10) * 10;
          }
          next[currentVertex.index] = point;
          const minX = Math.min(...next.map((item) => item.x));
          const minY = Math.min(...next.map((item) => item.y));
          const maxX = Math.max(...next.map((item) => item.x));
          const maxY = Math.max(...next.map((item) => item.y));
          store.updateWidget(currentVertex.widgetId, {
            x: Math.round(minX),
            y: Math.round(minY),
            width: Math.max(maxX - minX, 2),
            height: Math.max(maxY - minY, 2),
            points: next.map((item) => ({ x: item.x - minX, y: item.y - minY })),
          } as Partial<Widget>, { history: false });
          store.commitHistory();
          return;
        }
  ...
  // A seleção do vértice só é válida enquanto o mesmo polígono continuar
  // selecionado e o índice ainda existir.
  useEffect(() => {
    const selected = selectedPolygonVertexRef.current;
    const polygon = selectedIds.length === 1
      ? screen?.widgets.find((widget) => widget.id === selectedIds[0])
      : null;
    if (
      selected &&
      (polygon?.type !== 'polygon' ||
        polygon.locked ||
        polygon.id !== selected.widgetId ||
        selected.index < 0 ||
        selected.index >= polygon.points.length)
    ) {
  ...
  function polygonNextPoint(e: { clientX: number; clientY: number }): PolygonPoint | null {
    const p = sheetPoint(e);
    if (!p) return null;
  ...
  function finishPolygon() {
    const pts = polygonPts.filter((p, i) => i === 0 || p.x !== polygonPts[i - 1].x || p.y !== polygonPts[i - 1].y);
    setPolygonPts([]);
    ...
    const w = Math.max(maxX - minX, 2);
    const h = Math.max(maxY - minY, 2);
    const widget = buildDefaultWidget('polygon', minX, minY, w, h) as PolygonWidget;
    widget.points = pts.map((p) => ({ x: p.x - minX, y: p.y - minY }));
    addWidget(widget);
  }

  // Enter finaliza, Esc cancela qualquer desenho em andamento.
  useEffect(() => {
  ...
  }, [pipeDraw, polygonDraw, pipePts, polygonPts]);
  ...
  function polygonAbsPoints(polygon: PolygonWidget): PolygonPoint[] {
    return scaleScadaPolygonPoints(polygon.points, polygon.width, polygon.height, polygon.width, polygon.height)
      .map((p) => ({ x: p.x + polygon.x, y: p.y + polygon.y }));
  }

  function commitPolygonAbsPoints(id: string, absPts: PolygonPoint[]) {
    if (absPts.length < 3) return;
    const minX = Math.min(...absPts.map((p) => p.x));
    const minY = Math.min(...absPts.map((p) => p.y));
    ...
    updateWidget(id, {
      x: Math.round(minX), y: Math.round(minY), width, height,
      points: absPts.map((p) => ({ x: p.x - minX, y: p.y - minY })),
    } as Partial<Widget>, { history: false });
  }

  function startPolygonVertexDrag(e: React.MouseEvent, polygon: PolygonWidget, idx: number, absPtsInit?: PolygonPoint[]) {
    if (e.button !== 0) return;
    e.stopPropagation();
    e.preventDefault();
    selectPolygonVertex({ widgetId: polygon.id, index: idx });
    const absPts = absPtsInit ?? polygonAbsPoints(polygon);
    const startX = e.clientX, startY = e.clientY;
    const orig = { ...absPts[idx] };
    let moved = false;
    function onMove(ev: MouseEvent) {
      ...
      const ny = orig.y + (ev.clientY - startY) / zoom;
      moved = true;
      absPts[idx] = ev.altKey
        ? { x: Math.round(nx), y: Math.round(ny) }
        : { x: Math.round(nx / 10) * 10, y: Math.round(ny / 10) * 10 };
      commitPolygonAbsPoints(polygon.id, absPts);
    }
    function onUp() {
      if (moved) useEditorStore.getState().commitHistory();
      window.removeEventListener('mousemove', onMove);
  ...
  function insertPolygonVertex(e: React.MouseEvent, polygon: PolygonWidget, segIdx: number) {
    if (e.button !== 0) return;
    e.stopPropagation();
    e.preventDefault();
    const abs = polygonAbsPoints(polygon);
    const nextIdx = (segIdx + 1) % abs.length;
    const mid = {
      x: Math.round((abs[segIdx].x + abs[nextIdx].x) / 2),
      y: Math.round((abs[segIdx].y + abs[nextIdx].y) / 2),
    };
    const next = [...abs.slice(0, nextIdx), mid, ...abs.slice(nextIdx)];
    commitPolygonAbsPoints(polygon.id, next);
    startPolygonVertexDrag(e, polygon, nextIdx, next);
  }

  function removePolygonVertex(polygon: PolygonWidget, idx: number) {
    const abs = polygonAbsPoints(polygon);
    if (abs.length <= 3) return;
    commitPolygonAbsPoints(polygon.id, abs.filter((_, i) => i !== idx));
    useEditorStore.getState().commitHistory();
  }
  ...
  function onWidgetMouseDown(e: React.MouseEvent, widget: Widget) {
    ...
    // Em modo de desenho, os cliques passam para o canvas (adicionar vértice).
    if (pipeDraw || polygonDraw) return;
    // Travado: ignora clique/arraste na tela (não seleciona, move nem redimensiona).
    if (widget.locked || isPinnedNavWidget(widget)) return;
    e.stopPropagation();
  ...
  function startResize(e: React.MouseEvent, handle: string, widget: Widget) {
    if (previewMode || e.button !== 0 || widget.locked || isPinnedNavWidget(widget)) return;
    e.stopPropagation();
    e.preventDefault();
    resizeRef.current = {
      handle,
      startX: e.clientX,
      startY: e.clientY,
      o: { x: widget.x, y: widget.y, w: widget.width, h: widget.height },
      id: widget.id,
      source: widget.type === 'component-instance'
        ? { width: widget.width, height: widget.height, children: widget.children }
        : undefined,
    };
    let resized = false;
    function onMove(ev: MouseEvent) {
      ...
      if (widget.type === 'component-instance') {
        useEditorStore.getState().resizeComponentInstance(r.id, {
          x,
          y,
          width: w,
          height: h,
        }, { history: false, source: r.source });
      } else {
  ...
  /** Redimensiona somente a logo, mantendo a toolbar e seu lado ancorado. */
  function startLogoResize(e: React.MouseEvent, widget: Widget) {
    if (previewMode || e.button !== 0 || widget.locked || widget.type !== 'nav-toolbar') return;
    const logo = normalizeNavToolbarLogo(widget.logo);
    if (!logo) return;
    e.stopPropagation();
    e.preventDefault();
    logoResizeRef.current = { startX: e.clientX, startY: e.clientY, id: widget.id, logo };
    let resized = false;
    function onMove(ev: MouseEvent) {
      const r = logoResizeRef.current;
      if (!r) return;
      if (ev.buttons === 0) { onUp(); return; }
      const dx = (ev.clientX - r.startX) / zoom;
      const dy = (ev.clientY - r.startY) / zoom;
      const nextWidth = r.logo.position === 'right' ? r.logo.width - dx : r.logo.width + dx;
      const nextHeight = r.logo.height + dy;
      resized = true;
      updateWidget(r.id, {
        logo: {
          ...r.logo,
          width: normalizeNavToolbarLogoWidth(nextWidth),
          height: normalizeNavToolbarLogoHeight(nextHeight),
        },
      }, { history: false });
    }
    function onUp() {
      if (logoResizeRef.current && resized) useEditorStore.getState().commitHistory();
  ...
  function onCanvasMouseDown(e: React.MouseEvent) {
    ...
    if (e.button !== 0) return;
    // Modo de desenho: cada clique adiciona um vértice.
    if (pipeDraw) {
    ...
    if (polygonDraw) {
      const p = polygonNextPoint(e);
      if (p) setPolygonPts((prev) => [...prev, p]);
      return;
    }
  ...
  function onCanvasMouseMove(e: React.MouseEvent) {
    ...
    if (polygonDraw) {
      setPolygonCursor(polygonNextPoint(e));
      return;
    }
      ...
      <div className="flex flex-1 min-h-0 min-w-0 flex-col overflow-hidden bg-slate-900">
        {previewToolbar && (
          <div style={{ height: clampPinnedToolbarH(previewToolbar.height), flexShrink: 0 }}>
            <NavToolbarWidgetView widget={previewToolbar} currentScreenId={screen.id} currentScreenName={screen.name} onNavigate={previewNavigate} />
          </div>
        )}
        <div className="flex flex-1 min-h-0 min-w-0">
          {previewSidebar && (
            <div style={{ width: clampPinnedSidebarW(previewSidebar.width), flexShrink: 0 }}>
              <NavSidebarWidgetView widget={previewSidebar} currentScreenId={screen.id} currentScreenName={screen.name} onNavigate={previewNavigate} />
            </div>
          )}
  ...
  const logoHandleStyle = (widget: Widget, surfaceWidth: number, surfaceHeight = widget.height): React.CSSProperties | null => {
    const logo = widget.type === 'nav-toolbar' ? normalizeNavToolbarLogo(widget.logo) : undefined;
    if (!logo) return null;
    const handleSize = 20 / zoom;
    const edgeX = logo.position === 'right'
      ? surfaceWidth - 8 - logo.width
      : 8 + logo.width;
    const rawX = logo.position === 'right' ? edgeX : edgeX - handleSize;
    const logoBottom = (surfaceHeight - logo.height) / 2 + logo.height;
    const rawY = logoBottom - handleSize;
    const inset = 2 / zoom;
    const maxX = Math.max(inset, surfaceWidth - handleSize - inset);
    const maxY = Math.max(inset, surfaceHeight - handleSize - inset);
    return {
      position: 'absolute',
      left: Math.min(Math.max(rawX, inset), maxX),
      top: Math.min(Math.max(rawY, inset), maxY),
      width: handleSize,
      height: handleSize,
      cursor: logo.position === 'right' ? 'nesw-resize' : 'nwse-resize',
      zIndex: 100004,
    };
  };
  return (
    <div
      ...
      onDoubleClick={pipeDraw ? finishPipe : polygonDraw ? finishPolygon : undefined}
      onContextMenu={onCanvasContextMenu}
      style={pipeDraw || polygonDraw ? { cursor: 'crosshair' } : undefined}
      ...
      <div style={{ position: 'absolute', left: panOffset.x, top: panOffset.y, transform: `scale(${zoom})`, transformOrigin: '0 0' }}>
        ...
        {dockToolbar && (
          ...
          >
            ...
            {ownToolbarW && selectedIds.length === 1 && selectedIds[0] === ownToolbarW.id && (() => {
              const style = logoHandleStyle(ownToolbarW, dockSidebarW + screen.width, dockToolbarH);
              return style ? (
                <LogoResizeHandle
                  position={normalizeNavToolbarLogo(ownToolbarW.logo)!.position}
                  // startLogoResize só lê a ref durante o gesto do usuário.
                  // eslint-disable-next-line react-hooks/refs
                  onMouseDown={(e) => startLogoResize(e, ownToolbarW)}
                  style={style}
                />
              ) : null;
            })()}
        ...
        >
          ...
          {selWidget?.type === 'nav-toolbar' && !selWidget.locked && (() => {
            const style = logoHandleStyle(selWidget, selWidget.width);
            const logo = normalizeNavToolbarLogo(selWidget.logo);
            return style ? (
              <LogoResizeHandle
                position={logo!.position}
                // eslint-disable-next-line react-hooks/refs
                onMouseDown={(e) => startLogoResize(e, selWidget)}
                style={style}
              />
            ) : null;
          })()}
          ...
          {polygonDraw && (
            <svg width={screen.width} height={screen.height} style={{ position: 'absolute', left: 0, top: 0, pointerEvents: 'none', zIndex: 100001, overflow: 'visible' }}>
              {polygonPts.length > 0 && (
                <polygon
                  points={[...polygonPts, ...(polygonCursor ? [polygonCursor] : [])].map((p) => `${p.x},${p.y}`).join(' ')}
                  fill="rgba(34,211,238,0.14)" stroke="#22D3EE" strokeWidth={3 / zoom} strokeDasharray={`${8 / zoom} ${5 / zoom}`} strokeLinejoin="round"
                />
              )}
              {polygonPts.map((p, i) => (
                <circle key={i} cx={p.x} cy={p.y} r={4 / zoom} fill="#fff" stroke="#06B6D4" strokeWidth={1.5 / zoom} />
              ))}
              {polygonPts.length === 0 && polygonCursor && (
                <circle cx={polygonCursor.x} cy={polygonCursor.y} r={4 / zoom} fill="rgba(255,255,255,0.6)" stroke="#06B6D4" strokeWidth={1.5 / zoom} />
              )}
            </svg>
          )}
          {polygonDraw && (
            <div style={{ position: 'absolute', left: 8, top: 8, zIndex: 100002, pointerEvents: 'none' }} className="rounded bg-slate-900/90 border border-cyan-500/50 px-2 py-1 text-[10px] text-cyan-300">
              Polígono: clique para adicionar 3+ vértices — duplo clique/Enter finaliza, Esc cancela
            </div>
          )}
          ...
          {/* Vértices editáveis do polígono selecionado, incluindo o lado de fechamento. */}
          {!previewMode && !pipeDraw && !polygonDraw && selWidget?.type === 'polygon' && !selWidget.locked && (() => {
            const polygon = selWidget as PolygonWidget;
            const abs = polygonAbsPoints(polygon);
            return (
              <>
                {abs.map((p, i) => {
                  ...
                  return (
                    <div
                      key={`polygon-mid-${i}`}
                      title="Adicionar vértice"
                      // insertPolygonVertex also selects the new vertex through
                      // an event handler; it never reads the ref during render.
                      // eslint-disable-next-line react-hooks/refs
                      onMouseDown={(e) => insertPolygonVertex(e, polygon, i)}
                      style={{
                        position: 'absolute', left: (p.x + q.x) / 2 - 3.5 / zoom, top: (p.y + q.y) / 2 - 3.5 / zoom,
                        width: 7 / zoom, height: 7 / zoom, borderRadius: '50%',
                        background: 'rgba(6,182,212,0.35)', border: `${1 / zoom}px solid #06B6D4`,
                ...
                {abs.map((p, i) => (
                  <div
                    key={`polygon-v-${i}`}
                    data-testid={`scada-polygon-vertex-${i}`}
                    role="button"
                    tabIndex={0}
                    aria-label={`Vértice ${i + 1} do polígono`}
                    aria-pressed={selectedPolygonVertex?.widgetId === polygon.id && selectedPolygonVertex.index === i}
                    title={abs.length > 3 ? 'Arrastar vértice — duplo clique remove' : 'Arrastar vértice — mínimo de 3'}
                    // startPolygonVertexDrag/selectPolygonVertex only access refs
                    // after the mouse event has fired, not during render.
                    // eslint-disable-next-line react-hooks/refs
                    onMouseDown={(e) => startPolygonVertexDrag(e, polygon, i)}
                    // eslint-disable-next-line react-hooks/refs
                    onFocus={() => selectPolygonVertex({ widgetId: polygon.id, index: i })}
                    onDoubleClick={(e) => { e.stopPropagation(); removePolygonVertex(polygon, i); }}
                    style={{
                      position: 'absolute', left: p.x - 5 / zoom, top: p.y - 5 / zoom,
                      width: 10 / zoom, height: 10 / zoom, borderRadius: '50%',
                      background: selectedPolygonVertex?.widgetId === polygon.id && selectedPolygonVertex.index === i ? '#67E8F9' : '#fff',
                      border: `${(selectedPolygonVertex?.widgetId === polygon.id && selectedPolygonVertex.index === i ? 2 : 1.5) / zoom}px solid ${selectedPolygonVertex?.widgetId === polygon.id && selectedPolygonVertex.index === i ? '#FACC15' : '#06B6D4'}`,
                      boxShadow: selectedPolygonVertex?.widgetId === polygon.id && selectedPolygonVertex.index === i ? '0 0 0 2px rgba(250,204,21,0.35)' : undefined,
                      cursor: 'grab', zIndex: 100001,
                    }}
                  />
                ))}
          ...
          {!previewMode && selWidget && !selWidget.locked && selWidget.type !== 'pipe' && !isPinnedNavWidget(selWidget) && [
            ...
            <div
              key={hd.k}
              data-testid={`scada-resize-handle-${hd.k}`}
              aria-label={`Redimensionar pelo lado ${hd.k}`}
