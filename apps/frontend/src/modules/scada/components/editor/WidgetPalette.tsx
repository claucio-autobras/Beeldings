...
export interface PaletteCategory { id: string; label: string; icon: React.ReactNode; items: PaletteItem[] }

export const SCADA_WIDGET_CATEGORIES: PaletteCategory[] = [
  {
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'polygon-draw', type: 'polygon', label: 'Polígono — desenhar no canvas' },
...
export const SCADA_WIDGET_DEFAULT_SIZES: Partial<Record<WidgetType, { w: number; h: number }>> = {
...
interface Props {
  onDropWidget: (type: WidgetType, defaultSize: { w: number; h: number }, overrides?: Partial<Widget>, anchor?: { x: number; y: number }) => void;
  components: SavedComponent[];
  onInsertComponent: (id: string, anchor?: { x: number; y: number }) => void;
  onDeleteComponent: (id: string) => void;
  onStartPipeDraw?: () => void;
  onStartPolygonDraw?: () => void;
}

export function WidgetPalette({ onDropWidget, components, onInsertComponent, onDeleteComponent, onStartPipeDraw, onStartPolygonDraw }: Props) {
  ...
  function handleDragStart(e: React.DragEvent, item: PaletteItem) {
    const size = item.size ?? SCADA_WIDGET_DEFAULT_SIZES[item.type] ?? { w: 120, h: 80 };
  ...
  return (
    <aside className="flex min-h-0 flex-[3] flex-col bg-slate-800 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto py-2">
        ...
        {SCADA_WIDGET_CATEGORIES.map((cat) => (
          <div key={cat.id} className="mb-1">
                ...
                {cat.items.map((item) => {
                  ...
                  return (
                    <div
                      ...
                      onDoubleClick={() => onDropWidget(item.type, item.size ?? SCADA_WIDGET_DEFAULT_SIZES[item.type] ?? { w: 64, h: 64 }, item.overrides)}
            ...
            {open[cat.id] && cat.id !== 'icons' && (
              <ul className="mt-1 px-3 pb-1 space-y-0.5">
                {cat.items.map((item) => item.id === 'pipe-draw' || item.id === 'polygon-draw' ? (
                  <li key={item.id}>
                    <button
                      type="button"
                      onClick={() => item.id === 'pipe-draw'
                        ? (onStartPipeDraw ? onStartPipeDraw() : useEditorStore.getState().setPipeDraw(true))
                        : (onStartPolygonDraw ? onStartPolygonDraw() : useEditorStore.getState().setPolygonDraw(true))}
                  ...
                  <li key={item.id ?? item.type}>
                    <div
                      draggable
                      onDragStart={(e) => handleDragStart(e, item)}
                      onDoubleClick={() => onDropWidget(item.type, item.size ?? SCADA_WIDGET_DEFAULT_SIZES[item.type] ?? { w: 120, h: 80 }, item.overrides)}
