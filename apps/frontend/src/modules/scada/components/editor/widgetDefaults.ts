...
import { DEVICE_COUNTER_DEFAULT_ICON, createScadaHoverDefaults } from '../../types/scada.types';
...
export function buildDefaultWidget(type: WidgetType, x: number, y: number, width: number, height: number): Widget {
  const base = { id: `w-${nanoid(8)}`, x, y, width, height, opacity: 1, visible: true, zIndex: 2, visibility: defaultVisibility, hover: createScadaHoverDefaults() };
  ...
  switch (type) {
    ...
    case 'device-counter':
      return { ...base, type, mode: 'connectivity', deviceFilter: 'all', scope: 'site', deviceIds: [], pointTag: '', points: [], operator: 'eq', value: 1, format: 'fraction', label: 'Dispositivos online', showLabel: true, colorOn: '#22C55E', iconName: DEVICE_COUNTER_DEFAULT_ICON, textColor: '#FFFFFF', backgroundColor: 'rgba(15,23,42,0.7)', fontSize: 22, borderRadius: 8 };
    case 'line':
    ...
    case 'polygon':
      return {
        ...base, type, fillEnabled: true, fillColor: 'rgba(6,182,212,0.12)', strokeColor: '#06B6D4',
        strokeWidth: 2, borderRadius: 0, deviceId: '', tagStatus: '', stateRules: [],
        points: [{ x: 0, y: 0 }, { x: width, y: 0 }, { x: width * 0.82, y: height }, { x: width * 0.18, y: height }],
      };
    case 'image':
    ...
    case 'nav-sidebar':
      return { ...base, type, position: 'left', backgroundColor: 'rgba(15,23,42,0.95)', textColor: '#CBD5E1', activeColor: '#06B6D4', items: [], verticalAlign: 'top', contentAlign: 'left' };
    case 'nav-toolbar':
      return {
        ...base, type, position: 'top', backgroundColor: 'rgba(15,23,42,0.95)',
        textColor: '#CBD5E1', activeColor: '#06B6D4', items: [], contentAlign: 'left',
        verticalAlign: 'top', itemGap: 4, itemPaddingX: 12, itemPaddingY: 6, fontSize: 12,
      };
    case 'command-button':
