...
export function SetpointRingWidgetView({ widget: w, getValue, staticRender }: Props) {
  ...
  return (
    <div data-scada-hover-border-surface="true" data-scada-hover-content-surface={isTransparentColor(w.backgroundColor) ? undefined : 'true'} style={{
      ...
      <svg data-scada-hover-border-surface="true" width={SIZE} height={SIZE} viewBox={`0 0 ${SIZE} ${SIZE}`}>
        {/* Trilha */}
        ...
        <text data-scada-hover-text-surface="true" x={cx} y={cy + SIZE * 0.04} textAnchor="middle"
        ...
        {w.unit && (
          <text data-scada-hover-text-surface="true" x={cx + SIZE * 0.22 + valueText.length * SIZE * 0.055} y={cy - SIZE * 0.06} textAnchor="middle" fill={w.mutedColor} fontSize={SIZE * 0.08} fontWeight={600} fontFamily="Inter, sans-serif">
            {w.unit}
          </text>
        )}
        {w.showLabel && w.label && (
          <text data-scada-hover-text-surface="true" x={cx} y={cy + SIZE * 0.17} textAnchor="middle" fill={w.mutedColor} fontSize={SIZE * 0.065} fontWeight={600} letterSpacing="0.14em" fontFamily="Inter, sans-serif">
            {w.label.toUpperCase()}
          </text>
        )}
      </svg>
