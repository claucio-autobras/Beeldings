...
export function EquipmentWidgetView({ widget, getValue, staticRender }: Props) {
  ...
  const raw = staticRender || !bound ? null : getValue(widget.deviceId, widget.tagStatus);
  const isOffline = !staticRender && bound && raw === null;
  ...
  return (
    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', userSelect: 'none', gap: 4 }}>
      ...
      {widget.showLabel && (
        <span data-scada-hover-text-surface={widget.labelText.trim() ? 'true' : undefined} style={{ fontSize: widget.labelFontSize ?? 11, color: '#94A3B8', fontFamily: 'Inter, sans-serif', textAlign: 'center', maxWidth: '100%', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          {widget.labelText}
        </span>
      )}
    </div>
