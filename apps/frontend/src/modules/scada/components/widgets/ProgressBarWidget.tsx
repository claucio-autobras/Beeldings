...
export function ProgressBarWidgetView({ widget, getValue, staticRender }: Props) {
  ...
  return (
    <svg data-scada-hover-border-surface="true" width="100%" height="100%" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" style={{ userSelect: 'none' }}>
      <defs>
        <clipPath id={`pb${uid}`}>
      ...
      {/* Preenchimento */}
      {!isOffline && pct > 0 && (
        <rect data-scada-hover-fill-surface="true" clipPath={`url(#pb${uid})`} x={pad} y={y} width={fillW} height={barH} fill={color} />
      )}
      {/* Valor */}
      {widget.showValue && (
        <text data-scada-hover-text-surface="true" x={W / 2} y={H / 2} dominantBaseline="central" textAnchor="middle" fill="#F1F5F9" fontSize={fontSize} fontFamily="'Fira Code', monospace" fontWeight="600" style={{ paintOrder: 'stroke' }} stroke="rgba(2,6,23,0.5)" strokeWidth={2}>
