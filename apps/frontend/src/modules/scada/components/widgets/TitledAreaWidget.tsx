...
export function TitledAreaWidgetView({ widget }: Props) {
  ...
  if (position === 'inside') {
    ...
    return (
      <div data-scada-hover-border-surface="true" data-scada-hover-content-surface="true" style={{ width: '100%', height: '100%', position: 'relative', ...scadaBackgroundStyle(widget.fillColor), border: `1px solid ${widget.borderColor}`, borderRadius: widget.borderRadius, userSelect: 'none', display: 'flex', flexDirection: 'column', alignItems }}>
        <span data-scada-hover-text-surface="true" style={{ ...titleStyle, padding: `${Math.max(4, Math.round(fontSize * 0.5))}px 10px 0` }}>{title}</span>
      </div>
    );
  }
  ...
  return (
    <fieldset data-scada-hover-border-surface="true" data-scada-hover-content-surface="true" style={{ width: '100%', height: '100%', margin: 0, padding: 0, minWidth: 0, ...scadaBackgroundStyle(widget.fillColor), border: `1px solid ${widget.borderColor}`, borderRadius: widget.borderRadius, userSelect: 'none' }}>
      <legend data-scada-hover-text-surface="true" style={{ ...titleStyle, ...legendMargin, padding: '0 6px' }}>{title}</legend>
    </fieldset>
  );
}
