...
import {
  ...
  Wind, Wrench, Zap, ImageOff, MonitorSmartphone,
} from 'lucide-react';
...
export const SCADA_ICONS: Record<string, ScadaIconDef> = {
  ...
  // Mantém o desenho original do Contador de Dispositivos disponível no picker.
  devices: { label: 'Dispositivos', Icon: MonitorSmartphone, onColor: '#22C55E', keywords: 'contador equipamentos ativos devices' },
  // ── Pacote BMS: Elétrica ──
...
/** Resolve o ícone do Contador de Dispositivos sem quebrar telas antigas ou JSON inválido. */
export function getDeviceCounterIcon(name?: string): SvgIcon {
  return (name ? (NAV_ICONS[name]?.Icon ?? SCADA_ICONS[name]?.Icon) : undefined)
    ?? SCADA_ICONS[DEVICE_COUNTER_DEFAULT_ICON].Icon;
}
