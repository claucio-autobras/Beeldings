...
export function CameraWidgetView({ widget, getValue, getReading, devices, isEditor, staticRender }: Props) {
  ...
  return (
    ...
    >
      ...
      {widget.showLabel && (
        <div data-scada-hover-text-surface="true" style={{ fontSize: widget.labelFontSize ?? 11, lineHeight: 1.3, color: '#CBD5E1', textAlign: 'center', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '100%' }}>
      ...
      {open && canPopup && (
        <div
          ref={popupRef}
          className="scada-hover-exempt rounded-xl border border-border bg-card shadow-2xl"
          style={{
            position: 'absolute',
