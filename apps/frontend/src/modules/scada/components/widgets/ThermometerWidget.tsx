...
export function ThermometerWidgetView({ widget, getValue, staticRender }: Props) {
  ...
  return (
    <svg data-scada-hover-border-surface="true" width="100%" height="100%" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="xMidYMid meet" style={{ userSelect: 'none' }}>
      <defs>
        <clipPath id={`thm${uid}`}>
      ...
      {/* Líquido */}
      {!isOffline && pct > 0 && (
        <rect data-scada-hover-fill-surface="true" clipPath={`url(#thm${uid})`} x={cx - tubeW / 2} y={liqTop} width={tubeW} height={bulbCy - liqTop} fill={color} />
      )}
      {/* Bulbo */}
      <circle data-scada-hover-fill-surface="true" cx={cx} cy={bulbCy} r={bulbR} fill={isOffline ? '#1E293B' : color} stroke="#334155" strokeWidth={1} />
      {/* Valor */}
      {widget.showValue && (
        <text data-scada-hover-text-surface="true" x={cx} y={textH} textAnchor="middle" fill={!staticRender && isOffline ? '#475569' : '#F1F5F9'} fontSize={Math.min(W * 0.32, 13)} fontFamily="'Fira Code', monospace" fontWeight="600">
