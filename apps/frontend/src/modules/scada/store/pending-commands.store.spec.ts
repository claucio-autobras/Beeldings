...
describe('pending-commands.store (contrato do otimista)', () => {
  ...
  it('notifica somente assinantes do ponto alterado', () => {
    const tag1Listener = jest.fn();
    const tag2Listener = jest.fn();
    const unsubscribeTag1 = subscribePendingCommand('dev1', 'tag1', tag1Listener);
    const unsubscribeTag2 = subscribePendingCommand('dev1', 'tag2', tag2Listener);

    setPendingCommand('dev1', 'tag1', 5);
    expect(tag1Listener).toHaveBeenCalledTimes(1);
    expect(tag2Listener).not.toHaveBeenCalled();

    setPendingCommand('dev1', 'tag2', 8);
    expect(tag1Listener).toHaveBeenCalledTimes(1);
    expect(tag2Listener).toHaveBeenCalledTimes(1);
