...
import { hasScreenTelemetryBindings, type ScadaScreen } from '../types/scada.types';
 ...
 */
export function useViewer(screenId: string, siteId?: string): UseViewerResult {
  const { data: screen, isLoading } = useQuery<ScadaScreen>({
    queryKey: ['scada-screen', screenId, siteId ?? null],
    queryFn: () => getScreen(screenId, siteId),
  });
  ...
  const merged: ScreenDevice[] = useMemo(
    () => [...devices, ...cameras, ...virtualDevices],
    [devices, cameras, virtualDevices],
  );

  const telemetryEnabled = Boolean(screen && hasScreenTelemetryBindings(screen.widgets));
  const { getValue, getReading, getPointStatus, connected } = useScreenTelemetry(merged, telemetryEnabled);
