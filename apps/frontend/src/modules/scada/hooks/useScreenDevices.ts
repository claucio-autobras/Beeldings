...
import { getControllers, type Controller } from '@/modules/sca/services/sca.service';
...
export function useScreenDevices(
  ...
  // Controladoras de acesso (SCA) — mesmo padrão de câmeras/switches: buscar
  // por tenant e filtrar pelo gateway da tela. Os pontos de saúde (temperatura,
  // memória, CPU, uptime, status…) já vêm no payload de /sca/controllers.
  const { data: controllers = [], isLoading: loadingControllers } = useQuery<Controller[]>({
    queryKey: ['sca-controllers', tenantId],
    queryFn: () => getControllers(tenantId),
    enabled: Boolean(tenantId),
  });

  // NVRs/DVRs gerenciáveis — pontos escalares (STATUS/UPTIME/CPU/MEMORIA/
  // TEMPERATURA) de /cftv/nvrs, mesmo padrão de câmeras/switches.
  const { data: nvrs = [], isLoading: loadingNvrs } = useQuery<ManagedNvr[]>({
    queryKey: ['cftv-nvrs', tenantId],
    queryFn: () => getNvrs(tenantId),
    enabled: Boolean(tenantId),
  });

  const merged: ScreenDevice[] = useMemo(
    () => {
      ...
      const scopedSwitches = gatewayId
        ...
        : [];
      const scopedControllers = gatewayId ? controllers.filter((c) => c.gatewayId === gatewayId) : [];
      const scopedNvrs = gatewayId ? nvrs.filter((n) => n.gatewayId === gatewayId) : [];
      return [
        ...scoped,
        ...scopedCameras,
        ...scopedSwitches,
    ...
    [gatewayId, devices, cameras, switches, controllers, nvrs, virtualDevices],
  );
