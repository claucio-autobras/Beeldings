...
export default function ScadaListPage() {
  ...
  const [editingCover, setEditingCover] = useState<ScadaProject | null>(null);
  ...
  const cards = useMemo<ScadaProjectCardData[]>(() => {
    // Match both tenant and site. Site IDs are currently global UUIDs, but
    // keeping the tenant in the key prevents a future/API inconsistency from
    // displaying another client's location on a card.
    const siteMap = new Map(sites.map((s) => [`${s.tenantId}:${s.id}`, s]));
    ...
    return scadaProjects
      ...
      .map((p) => {
        const site = siteMap.get(`${p.tenantId}:${p.siteId}`);
        return {
          projectId: p.id,
          projectName: p.name,
          siteName: site?.name,
  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {cards.map((c) => (
            <ProjectCard
              ...
              canEdit={canEdit}
              onEdit={() => {
                const project = scadaProjects.find((p) => p.id === c.projectId);
                if (project) setEditingCover(project);
              }}
      ...
      {editingCover && (
        <ProjectCoverModal
          project={editingCover}
          onClose={() => setEditingCover(null)}
          onSaved={(updatedProject) => {
            qc.setQueryData<ScadaProject[]>(
              ['scada-projects', tenantScope ?? 'all'],
              (current) => current?.map((project) => project.id === updatedProject.id ? updatedProject : project),
            );
            void qc.invalidateQueries({ queryKey: ['scada-projects'] });
          }}
        />
      )}
    </div>
