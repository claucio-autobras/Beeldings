...
interface Props {
  screenId: string;
  /** Site currently selected by a client; used to enforce the same scope server-side. */
  siteId?: string;
  /** Renderiza embutido no layout do BlueBee (sem barra de voltar/standalone). */
  embedded?: boolean;
}

/** Destino contextual do retorno do viewer standalone. */
export function getViewerBackHref(projectId: string | null | undefined): string {
  return projectId ? `/scada/project/${encodeURIComponent(projectId)}` : '/scada';
}

export default function ScadaViewerPage({ screenId, siteId, embedded = false }: Props) {
  ...
  const { screen, loading, connected, getTagValue, getTagReading, getTagStatus, getGroupAggregate, devices } = useViewer(currentId, siteId);
      ...
      <header className="flex h-11 shrink-0 items-center justify-between border-b border-slate-800 px-4">
        <div className="flex items-center gap-3 min-w-0">
          {!embedded && (
            <button
              type="button"
              onClick={() => router.push(getViewerBackHref(screen?.projectId))}
