...
import type { AlarmEventItem } from '@/modules/alarms/services/alarms-api.service';
...
// Status"; `pendingAck` (apenas NORMALIZED_UNACK) alimenta o card
// "Aguard. Reconhecimento".
...
/** Eventos ainda ativos, já ordenados por criticidade e atividade no backend. */
export async function getRecentActiveAlarms(
  tenantId?: string,
  siteId?: string,
  limit = 5,
): Promise<AlarmEventItem[]> {
  const params = new URLSearchParams({ open: 'true', limit: String(limit) });
  if (tenantId) params.set('tenantId', tenantId);
  if (siteId) params.set('siteId', siteId);
  return apiGet<AlarmEventItem[]>(`/alarm-events?${params.toString()}`);
}
...
/**
 * Origem de uma queda de comunicação dentro de um bucket: cliente + dispositivo/
 * gateway já classificado pela tela de destino correta. Alimenta a quebra por
 * origem do tooltip e o roteamento de clique da barra de "Quedas (offline)".
 */
...
// ─── Visão geral do site (energia, água e funcionamento) ─────────────────────

export type SiteOverviewStatus = 'ready' | 'no_data' | 'not_configured';

export interface SiteOverviewSeriesPoint {
  timestamp: string;
  value: number;
}

export interface SiteOverviewSource {
  pointId: string;
  pointName: string;
  deviceName: string;
  siteName: string | null;
...
export interface SiteConsumptionMetric {
  status: SiteOverviewStatus;
  total: number | null;
  unit: string;
  series: SiteOverviewSeriesPoint[];
  configuredPoints: number;
  sources: SiteOverviewSource[];
}

export interface SiteRuntimeItem {
  pointId: string;
  pointName: string;
  deviceId: string;
  deviceName: string;
...
export interface SiteOverviewData {
  ...
  to: string;
  generatedAt: string;
  energy: SiteConsumptionMetric;
  water: SiteConsumptionMetric;
  runtime: {
    status: SiteOverviewStatus;
    totalHours: number | null;
    unit: 'h';
    configuredPoints: number;
    items: SiteRuntimeItem[];
    series: SiteOverviewSeriesPoint[];
  };
}
...
export async function getSiteOverview(
  ...
  tenantId?: string,
  siteId?: string,
): Promise<SiteOverviewData> {
  const params = new URLSearchParams({ period });
  if (tenantId) params.set('tenantId', tenantId);
  if (siteId) params.set('siteId', siteId);
  return apiGet<SiteOverviewData>(`/dashboard/site-overview?${params.toString()}`);
}
