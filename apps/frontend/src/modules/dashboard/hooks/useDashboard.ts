...
export function useSiteOverview(params: {
  period: DashboardPeriod;
  tenantId?: string;
  siteId?: string;
}) {
  const { period, tenantId, siteId } = params;
  return useQuery({
    queryKey: ['dashboard-site-overview', tenantId ?? 'all', siteId ?? 'all', period],
    queryFn: () => getSiteOverview(period, tenantId, siteId),
    refetchInterval: 60_000,
    staleTime: 30_000,
  });
}
...
export function useRecentActiveAlarms(params: {
  tenantId?: string;
  siteId?: string;
  limit?: number;
}) {
  const { tenantId, siteId, limit = 5 } = params;
  return useQuery({
    queryKey: ['dashboard-recent-active-alarms', tenantId ?? 'all', siteId ?? 'all', limit],
    queryFn: () => getRecentActiveAlarms(tenantId, siteId, limit),
    refetchInterval: 30_000,
    staleTime: 10_000,
  });
}
