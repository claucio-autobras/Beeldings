...
/**
 * KPI de ação (Alarmes Ativos / Aguard. Reconhecimento / Disp. Offline): barra de status,
...
export function ActionKpiCard({ title, value, status, icon, href, spark, trend, isLoading }: ActionKpiCardProps) {
  ...
  if (isLoading) {
    return (
      <div className="chamfer bg-border p-px animate-pulse">
        <div className="chamfer-in relative overflow-hidden bg-card p-4">
  ...
  return (
    <div className="chamfer chamfer-clickable group bg-border p-px">
      <Link
        href={href}
        className="chamfer-in chamfer-hover-content relative block overflow-hidden bg-card p-4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500"
      >
