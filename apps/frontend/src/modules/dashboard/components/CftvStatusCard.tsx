...
export function CftvStatusCard({ kpi }: CftvStatusCardProps) {
  ...
  const seeCameras = (
    <Link
      href="/cftv"
      className="dashboard-clickable mt-3 block w-full rounded-md border border-border bg-muted/40 py-1.5 text-center text-xs font-medium text-foreground transition-colors hover:bg-muted"
    >
      {t('Ver Câmeras')}
    </Link>
  );
          ...
          {kpi.cameras.map((cam) => (
            ...
            >
              <div className="flex items-center justify-between gap-2">
                ...
                <span
                  className={`flex shrink-0 items-center gap-1.5 rounded-full border px-2 py-0.5 text-xs font-medium ${HEALTH_BADGE_CLASS[cam.health]}`}
                >
              ...
              {cam.health === 'online' && (
                <div className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1.5 text-xs tabular-nums text-muted-foreground">
            ...
            {kpi.cameras.slice(0, MAX_ROWS).map((cam) => (
              <li key={cam.id} className="flex items-center gap-2 py-1.5 first:pt-0 last:pb-0">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {kpi.multiSite && (
                    <div className="truncate text-xs text-muted-foreground" title={cam.site}>
                      {cam.site}
                    </div>
                  )}
                </div>
