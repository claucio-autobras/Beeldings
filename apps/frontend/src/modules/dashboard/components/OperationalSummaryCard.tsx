...
import { HexMeter } from '@/components/ui/HexMeter';
...
/**
 * "Resumo Operacional" (visão Cliente): críticos ativos, aguardando reconhecimento,
...
export function OperationalSummaryCard({
  ...
  const rowCls =
    'flex items-center justify-between chamfer-sm border border-border bg-muted/30 p-3';
  const interactiveRowCls = `${rowCls} chamfer-clickable`;
  ...
  return (
    <div className="chamfer-sm border border-border bg-card shadow-sm">
      ...
      <div className="space-y-3 p-4">
        <Link href="/alarms?severity=HIGH&state=open" className={interactiveRowCls}>
        ...
        <Link href="/alarms?state=NORMALIZED_UNACK" className={interactiveRowCls}>
          <span className="flex items-center gap-2 text-xs text-muted-foreground">
            <Clock size={15} className="text-orange-500" /> {t('Aguardando reconhecimento')}
          </span>
        ...
        >
          <span className="flex items-center gap-2 text-xs text-muted-foreground">
           ...
           >
             {availabilityPct !== null && availabilityPct !== undefined && (
               <HexMeter value={availabilityPct} cells={5} label={t('Disponibilidade')} />
             )}
             <span
