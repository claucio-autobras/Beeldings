...
export type HealthMetric =
  "cpu" | "memory" | "ram_total" | "storage" | "temperature" | "packet_loss";
...
export interface OidProfile {
  ...
  label: string;
  oids: Partial<
    Record<HealthMetric, { oid: string; scale: number; unit: string }>
  >;
}
...
export async function getCameras(tenantId?: string): Promise<Camera[]> {
  return apiGet<Camera[]>(
    `/cftv/cameras${tenantId ? `?tenantId=${tenantId}` : ""}`,
  );
}
  ...
  if (!data.success) {
    throw new Error(
      (data as { error?: string }).error ??
        "Erro desconhecido no probe de capacidades.",
    );
  }
...
export async function deleteCamera(
  ...
  await apiDelete(`/cftv/cameras/${id}`, {
...
export async function scanOnvifNetwork(params: {
  ...
  if (!data.success) {
    throw new Error(
      data.error ?? "Erro desconhecido ao descobrir câmeras ONVIF.",
    );
  }
  return data.devices ?? [];
}
 ...
 */
export async function testCameraSnmp(
  params: SnmpV3Input & {
    tenantId: string;
    gatewayId: string;
    ip: string;
...
export type MetricProposalState =
  "active" | "broken" | "suggested" | "unavailable";
...
/**
 * Status durável do job de diagnóstico (mesma forma devolvida pelo backend
 * em GET .../diagnose/:diagnoseId/progress) — job sobrevive a múltiplas
 * instâncias do backend e nunca fica "pending" para sempre: termina em
 * 'done' ou 'error' dentro do timeout de segurança do backend (120s).
 */
export type SnmpDiagnoseJobStatus =
  | { status: "unknown" }
  | { status: "pending"; progress: SnmpDiagnoseProgress }
  | { status: "done"; result: SnmpDiagnoseOutcome }
...
/**
 * Tempo total de polling no frontend — folga sobre o timeout de segurança
 * do backend (120s) para nunca desistir antes dele.
 */
const DIAGNOSE_POLL_TIMEOUT_MS = 130_000;
const DIAGNOSE_POLL_INTERVAL_MS = 1500;
/** Tolerância a respostas "unknown" seguidas — corrida logo após o POST. */
const MAX_CONSECUTIVE_UNKNOWN = 5;

/**
 * Acompanha por polling um diagnóstico SNMP já iniciado até ele terminar
 * (sucesso ou erro) ou estourar o tempo total. Nenhuma requisição HTTP desta
 * função fica aguardando o gateway — cada tentativa é uma consulta rápida ao
 * status durável do job. Compartilhado por CFTV e SCA (mesmo contrato).
 */
export async function pollSnmpDiagnoseJob(
  diagnoseId: string,
  fetchStatus: (diagnoseId: string) => Promise<SnmpDiagnoseJobStatus>,
): Promise<SnmpDiagnoseOutcome> {
  const startedAt = Date.now();
  let consecutiveUnknown = 0;
  while (Date.now() - startedAt < DIAGNOSE_POLL_TIMEOUT_MS) {
    let status: SnmpDiagnoseJobStatus;
    try {
      status = await fetchStatus(diagnoseId);
    } catch {
    ...
    consecutiveUnknown =
      status.status === "unknown" ? consecutiveUnknown + 1 : 0;
    if (consecutiveUnknown > MAX_CONSECUTIVE_UNKNOWN) {
      throw new Error(
        "O gateway não respondeu ao diagnóstico SNMP. Verifique a conexão do gateway e tente novamente.",
      );
    }
    await new Promise((resolve) =>
      setTimeout(resolve, DIAGNOSE_POLL_INTERVAL_MS),
    );
  }
  throw new Error(
    "O diagnóstico SNMP excedeu o tempo limite. O gateway pode estar sobrecarregado — tente novamente.",
  );
}
 ...
 * resumido. Pode levar até ~2 min. O POST só confirma o início (evita
 * conexões HTTP longas atrás do proxy de produção) — o resultado final é
 * obtido por polling. Falha imediata se o gateway está offline.
 */
export async function diagnoseCameraSnmp(
  ...
  const started = await apiPost<SnmpDiagnoseStartResponse>(
    `/cftv/cameras/${cameraId}/diagnose-snmp`,
    { diagnoseId: id },
  );
  if (!started.success) {
  ...
  return pollSnmpDiagnoseJob(started.diagnoseId, (diagId) =>
    apiGet<SnmpDiagnoseJobStatus>(`/cftv/diagnose/${diagId}/progress`),
  );
}
...
export async function getDiagnoseProgress(
  ...
  const status = await apiGet<SnmpDiagnoseJobStatus>(
...
export async function applySnmpOids(
  ...
  return apiPost<Camera>(`/cftv/cameras/${cameraId}/apply-snmp-oids`, {
    ...
    ...(metricConfidence && Object.keys(metricConfidence).length
      ? { metricConfidence }
      : {}),
    ...(diagnoseId?.trim() ? { diagnoseId: diagnoseId.trim() } : {}),
  });
}
...
export async function testCameraOid(
  ...
  return apiPost<LiveOidTestOutcome>(`/cftv/cameras/${cameraId}/test-oid`, {
    oid,
  });
}
...
export async function getSwitches(tenantId?: string): Promise<ManagedSwitch[]> {
  return apiGet<ManagedSwitch[]>(
    `/cftv/switches${tenantId ? `?tenantId=${tenantId}` : ""}`,
  );
}
...
export async function deleteSwitch(
  ...
  await apiDelete(`/cftv/switches/${id}`, {
    headers: sensitiveActionHeaders(confirmationToken),
  });
}
...
export async function syncSwitchPorts(id: string): Promise<SwitchSyncResult> {
  const data = await apiPost<
    { success: boolean; error?: string } & Partial<SwitchSyncResult>
  >(`/cftv/switches/${id}/sync-ports`, {});
  if (!data.success)
    throw new Error(data.error ?? "Erro ao sincronizar portas.");
  return {
    added: data.added ?? 0,
...
export async function probeSwitchCapabilities(
  id: string,
): Promise<ProbeCapabilitiesResult> {
  const data = await apiPost<
    | ({ success: true } & ProbeCapabilitiesResult)
  ...
  if (!data.success) {
    throw new Error(
      (data as { error?: string }).error ?? "Erro desconhecido no probe.",
    );
  }
  return data as ProbeCapabilitiesResult;
}

export async function getSwitchCapabilities(
  id: string,
): Promise<MetricCapability[]> {
...
export async function getNvrs(tenantId?: string): Promise<ManagedNvr[]> {
  return apiGet<ManagedNvr[]>(
    `/cftv/nvrs${tenantId ? `?tenantId=${tenantId}` : ""}`,
  );
}
...
export async function updateNvr(
  id: string,
  data: NvrInput,
): Promise<ManagedNvr> {
...
export async function deleteNvr(
  id: string,
  confirmationToken: string,
): Promise<void> {
  await apiDelete(`/cftv/nvrs/${id}`, {
    headers: sensitiveActionHeaders(confirmationToken),
  });
}
...
export async function syncNvrDisks(id: string): Promise<NvrSyncResult> {
  const data = await apiPost<
    { success: boolean; error?: string } & Partial<NvrSyncResult>
  >(`/cftv/nvrs/${id}/sync-disks`, {});
  if (!data.success)
    throw new Error(data.error ?? "Erro ao sincronizar discos/canais.");
  return {
    added: data.added ?? 0,
...
export async function probeNvrCapabilities(
  id: string,
): Promise<ProbeCapabilitiesResult> {
  const data = await apiPost<
    | ({ success: true } & ProbeCapabilitiesResult)
...
export async function getNvrCapabilities(
  id: string,
): Promise<MetricCapability[]> {
...
export async function keepAliveLiveView(
  sessionId: string,
...
export async function setCameraCritical(
  cameraId: string,
  critical: boolean,
): Promise<void> {
