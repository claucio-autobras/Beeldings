...
describe('buildHealthTiles', () => {
  ...
  it('nunca renderiza uma disponibilidade em bytes rotulada como percentual (Control iD/iDFlex)', () => {
    // Evidência do bug: um binding legado grava unit '%' num ponto cujo valor
    // cru na verdade é uma capacidade em bytes (~60 MB) — nunca deve virar
    // "63107072%" nem gerar barra de 100%.
    const tiles = buildHealthTiles([point('memory_available', '%')], () => ({ value: 63_107_072 }));
    expect(tiles).toEqual([
  ...
  it('rejeita memory_used_percent explícito fora da faixa plausível', () => {
    const tiles = buildHealthTiles([point('memory_used_percent', '%')], () => ({ value: 63_107_072 }));
    expect(tiles[0]).toMatchObject({ key: 'memory_used', text: null, pct: null, emptyState: 'sem dados' });
  });
  ...
  it('exibe os 256 MB da RAM Hikvision após a migração do ponto legado para bytes', () => {
    // O publisher converte o DevicePoint legado (unit MB) para bytes antes da
    // telemetria canônica chegar; o card usa o metadado persistido atualizado.
    const tiles = buildHealthTiles(
      [point('ram_total', 'bytes')],
      () => ({ value: 256 * 1024 * 1024 }),
    );

    expect(tiles).toEqual([
      expect.objectContaining({ key: 'ram_total', text: '256 MB' }),
    ]);
  });
...
describe('buildHealthTiles — staleness (SNMP: valores que somem sozinhos)', () => {
  it('marca a tile como desatualizada quando a leitura é velha, mas mantém o último valor', () => {
    const oldTimestamp = new Date(Date.now() - 10 * 60 * 1000).toISOString(); // 10 min atrás
    const tiles = buildHealthTiles(
      [point('cpu', '%')],
      () => ({ value: 42, timestamp: oldTimestamp }),
    );
    expect(tiles.find((tile) => tile.key === 'cpu')).toMatchObject({
      text: '42%',
      stale: true,
    });
  });

  it('não marca como desatualizado quando a leitura é recente', () => {
    const recentTimestamp = new Date(Date.now() - 30 * 1000).toISOString(); // 30s atrás
    const tiles = buildHealthTiles(
      [point('cpu', '%')],
      () => ({ value: 42, timestamp: recentTimestamp }),
    );
    expect(tiles.find((tile) => tile.key === 'cpu')).toMatchObject({
      text: '42%',
      stale: false,
  ...
  it('nunca marca "sem dados" como desatualizado (não há valor velho sem valor)', () => {
    const tiles = buildHealthTiles([point('temperature', '°C')], () => null);
    expect(tiles.find((tile) => tile.key === 'temperature')).toMatchObject({
      text: null,
      stale: false,
    });
  ...
  it('combina a staleness de memória disponível + total na tile derivada', () => {
    const oldTimestamp = new Date(Date.now() - 10 * 60 * 1000).toISOString();
    const recentTimestamp = new Date().toISOString();
    const tiles = buildHealthTiles(
      [point('memory_available', 'bytes'), point('ram_total', 'bytes')],
      (tag) =>
        tag === 'MEMORY_AVAILABLE'
          ? { value: 61_028 * 1024, timestamp: oldTimestamp }
          : { value: 119_424 * 1024, timestamp: recentTimestamp },
    );
    // Disponível está velha — a tile derivada (memória usada) deve refletir isso
    // mesmo com o total fresco, pois ambos compõem o percentual mostrado.
    expect(tiles.find((tile) => tile.key === 'memory_used')).toMatchObject({ stale: true });
  });
});
