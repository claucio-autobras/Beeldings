  ...
  return (
    <div ref={ref} className="relative">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="dashboard-clickable flex items-center gap-1.5 rounded-md border border-amber-300 bg-amber-50 px-2.5 py-1.5 text-xs font-medium text-amber-700 transition-colors hover:bg-amber-100 focus:outline-none focus:ring-2 focus:ring-amber-400/50"
        aria-expanded={open}
        aria-haspopup="true"
      >
        ...
        >
          ...
          {options.map((opt) => (
            <button
              ...
              role="menuitem"
              onClick={opt.action}
              className="dashboard-clickable block w-full px-3 py-2.5 text-left transition-colors hover:bg-muted focus:bg-muted focus:outline-none"
            >
