...
export default function UsersPage() {
  ...
  const deleteMutation = useMutation({
    mutationFn: ({ id, confirmationToken }: { id: string; confirmationToken: string }) =>
      apiDelete(`/auth/users/${id}`, { headers: sensitiveActionHeaders(confirmationToken) }),
    onSuccess: () => {
              ...
              {users.map((u) => (
                <tr key={u.id} className="transition-colors duration-150 hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3.5 text-right">
                    {/* ADMIN pode excluir qualquer outro usuário, inclusive outro
                        ADMIN. A própria conta nunca recebe a ação. */}
                    {u.id !== currentUser.id &&
      ...
      {confirmDeleteId && (
        <PasswordConfirmDialog
          ...
          onConfirm={(confirmationToken) => {
            deleteMutation.mutate({ id: confirmDeleteId, confirmationToken });
          }}
        />
      )}
    </div>
