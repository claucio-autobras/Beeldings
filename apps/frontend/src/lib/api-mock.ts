...
const mockScadaProjects: MockScadaProject[] = [];
...
function handleGet(pathname: string, params: URLSearchParams): unknown {
  ...
  if (pathname === '/scada/projects') {
    return mockScadaProjects.filter((p) => !tenantId || p.tenantId === tenantId);
  }

  // Clientes (tenants)
  if (pathname === '/tenants') {
    const profile = mockProfile();
    const isGlobal = profile.role === 'ADMIN' || profile.role === 'CCO' || profile.role === 'SUPERVISOR';
    const visibleTenants = isGlobal || !profile.tenantId
      ? mockTenants
      : mockTenants.filter((t) => t.id === profile.tenantId);
  ...
  if (pathname === '/sites') {
    return mockSites
      ...
      .map((s) => ({
        ...
        name: s.name,
        tenantId: s.tenantId,
        location: (s as { location?: string | null }).location ?? null,
        createdAt: nowIso(),
        _count: { projects: 0 },
      }));
  }
...
function handlePost(pathname: string, body: unknown): unknown {
  ...
  // Adição de projeto SCADA — mantém uma resposta completa e estado local
  // para que o modo demonstração reflita a capa após invalidar a query.
  if (pathname === '/scada/projects') {
    const b = (body ?? {}) as { projectId?: string; coverImageDataUrl?: string };
    const projectId = b.projectId ?? genId();
    const existing = mockScadaProjects.find((p) => p.id === projectId);
    const project = existing ?? {
      id: projectId,
      name: 'Projeto SCADA',
      siteId: '',
      tenantId: mockProfile().tenantId ?? 'tenant-autobras',
      coverImageUrl: null,
    };
    if (Object.prototype.hasOwnProperty.call(b, 'coverImageDataUrl')) {
      project.coverImageUrl = b.coverImageDataUrl ?? null;
    }
    if (!existing) mockScadaProjects.push(project);
    return project;
  }
...
function handlePatch(pathname: string, body: unknown): unknown {
  // Atualização da capa do projeto SCADA — mantém o restante do projeto e
  // reflete a alteração no modo demonstração.
  const projectCoverMatch = pathname.match(/^\/scada\/projects\/([^/]+)$/);
  if (projectCoverMatch) {
    const project = mockScadaProjects.find((p) => p.id === projectCoverMatch[1]);
    if (!project) throw new Error(`Projeto não encontrado: ${projectCoverMatch[1]}`);
    const b = (body ?? {}) as { coverImageDataUrl?: string | null };
    if (b.coverImageDataUrl !== undefined) project.coverImageUrl = b.coverImageDataUrl;
    return project;
  }
...
export function mockRequest<T>(method: HttpMethod, path: string, body?: unknown): Promise<T> {
  ...
  switch (method) {
    ...
    case 'DELETE':
      if (pathname.startsWith('/scada/projects/')) {
        const projectId = pathname.split('/').filter(Boolean).pop();
        const index = mockScadaProjects.findIndex((p) => p.id === projectId);
        if (index >= 0) mockScadaProjects.splice(index, 1);
      }
