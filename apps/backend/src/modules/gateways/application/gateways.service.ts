...
type GatewayLiveWithHealth = Gateway & {
...
/** Gateway enriquecido para o resumo global, incluindo cliente e sites. */
export type GatewayWithHealth = GatewayLiveWithHealth & {
  /** Nome do cliente e sites atendidos, usados no resumo global do Dashboard. */
  tenantName: string;
  siteNames: string[];
};
...
export class GatewaysService {
   ...
   */
  private withLiveStatus(gateway: Gateway): GatewayLiveWithHealth {
  ...
  private siteNames(
    projects: Array<{ site: { name: string } }>,
    devices: Array<{ site: { name: string } | null }>,
  ): string[] {
    return [...new Set([
      ...projects.map((project) => project.site.name),
      ...devices.flatMap((device) => (device.site ? [device.site.name] : [])),
    ])];
  }
  ...
  async findAll(tenantId: string | undefined): Promise<GatewayWithHealth[]> {
    const gateways = await this.prisma.gateway.findMany({
      ...
      include: {
        ...
        projects: { select: { site: { select: { name: true } } } },
        devices: { select: { site: { select: { name: true } } } },
      },
    });
    return this.attachDeviceNames(
      gateways.map(({ tenant, projects, devices, ...gateway }) =>
        this.withContext(
          gateway,
          tenant.name,
          this.siteNames(projects, devices),
        ),
      ),
      tenantId,
    );
  ...
  async findOne(id: string, tenantId: string | undefined): Promise<GatewayWithHealth> {
    ...
    const { tenant, projects, devices, ...base } = gateway;
    const [enriched] = await this.attachDeviceNames(
      [this.withContext(base, tenant.name, this.siteNames(projects, devices))],
      tenantId,
    );
    return enriched;
  }
