...
describe('CftvController — monitoredDeviceType em criação de câmera', () => {
  ...
  describe('createCamera (SNMP)', () => {
    ...
    it('persiste perfil Hikvision manual e seus OIDs homologados já no cadastro', async () => {
      const prismaMock = buildPrismaMock();
      const controller = buildController(prismaMock);
      ...
      const createArg = prismaMock.device.create.mock.calls[0][0] as {
        data: {
          ...
          points: { create: Array<{ tag: string; binding: { oid: string } }> };
        };
      };
      ...
      expect(createArg.data.points.create).toEqual(expect.arrayContaining([
        expect.objectContaining({
          tag: 'CPU',
          binding: expect.objectContaining({ oid: '1.3.6.1.4.1.39165.1.7.0' }),
        }),
        expect.objectContaining({
          tag: 'RAM_TOTAL',
          binding: expect.objectContaining({ oid: '1.3.6.1.4.1.39165.1.10.0' }),
        }),
        expect.objectContaining({
          tag: 'ARMAZENAMENTO',
          binding: expect.objectContaining({ oid: '1.3.6.1.4.1.39165.1.9.0' }),
        }),
      ]));
    });
