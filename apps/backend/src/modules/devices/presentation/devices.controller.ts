...
const SITE_METRIC_KINDS = new Set(['energy', 'water', 'runtime']);
const SITE_METRIC_MODES = new Set(['cumulative', 'rate', 'state']);
const DIGITAL_BACNET_TYPES = new Set(['BI', 'BO', 'BV', 'MSI', 'MSO', 'MSV']);

function isDigitalPoint(protocol: string, objectType: string, binding: unknown): boolean {
  const b = (binding ?? {}) as Record<string, unknown>;
  if (protocol === 'bacnet') return DIGITAL_BACNET_TYPES.has(objectType);
  if (protocol === 'modbus') {
    return b.registerType === 'coil' || b.registerType === 'discrete' || b.dataType === 'boolean';
  }
  return b.valueType === 'boolean';
}

function normalizeSiteMetricUnit(
  metric: string,
  mode: string,
  ...
  const aliases: Record<string, string> = {
    wh: 'Wh', kwh: 'kWh', mwh: 'MWh', w: 'W', kw: 'kW', mw: 'MW',
    l: 'L', m3: 'm3', 'm³': 'm³',
    'l/s': 'L/s', 'l/min': 'L/min', 'l/h': 'L/h',
    'm3/s': 'm3/s', 'm3/min': 'm3/min', 'm3/h': 'm3/h',
    'm³/s': 'm³/s', 'm³/min': 'm³/min', 'm³/h': 'm³/h',
  };
  const normalized = aliases[value.toLowerCase()] ?? value;
  const allowed = metric === 'energy'
    ? (mode === 'cumulative' ? ['Wh', 'kWh', 'MWh'] : ['W', 'kW', 'MW'])
    : mode === 'cumulative'
      ? ['L', 'm3', 'm³']
      : ['L/s', 'L/min', 'L/h', 'm3/s', 'm3/min', 'm3/h', 'm³/s', 'm³/min', 'm³/h'];
  if (!allowed.includes(normalized)) {
    throw new BadRequestException(
      `Unidade inválida para ${metric} (${mode}). Use: ${allowed.join(', ')}`,
    );
  }
  return normalized;
}
...
export class DevicesController {
  ...
  /**
   * GET /devices/:id/mqtt-connection
   *
   * Diagnóstico objetivo da credencial MQTT dedicada: diferencia credencial
   * ausente, usuário inexistente no EMQX e usuário existente sem sessão ativa.
   * A senha nunca é retornada por esta rota.
   */
  @Get(':id/mqtt-connection')
  @UseGuards(JwtAuthGuard)
  async getMqttConnection(
    ...
    if (device.protocol !== 'mqtt') {
      throw new BadRequestException('O diagnóstico de conexão MQTT só se aplica a dispositivos MQTT');
    }

    const cfg = this.mqttDeviceConfig(device);
    const username = cfg.deviceMqttUser?.trim() ?? '';
    if (!username) {
      return {
        available: true,
        connected: false,
    ...
    return {
      ...await this.emqxProvisioning.getMqttClientConnection(username),
      username,
      broker: process.env.MQTT_BROKER_URL ?? null,
      rootTopic: cfg.rootTopic ?? null,
    };
    ...
    return {
      ...
      points: device.protocol === 'modbus'
        ...
        : device.points.map((p) => ({
            ...
            siteMetric: p.siteMetric ?? null,
            siteMetricMode: p.siteMetricMode ?? null,
            value: false,
            status: 'normal',
  ...
  async createMqttDevice(
    ...
    if (isRootMode && !this.emqxProvisioning.isConfigured()) {
      throw new ServiceUnavailableException(
        'O cadastro MQTT em tópico raiz exige o provisionamento do broker EMQX; configure EMQX_API_URL e tente novamente',
      );
    }
    ...
    // subscribe do gateway nos namespaces raiz. Este é um pré-requisito do
    // cadastro: nunca retornar um device sem a credencial que a tela promete.
    if (isRootMode && rootTopic) {
      ...
      try {
        ...
        // A criação no banco ainda não foi exposta ao cliente com sucesso.
        // Limpe eventuais recursos parciais no EMQX e remova o órfão local,
        // para que uma nova tentativa possa reutilizar o tópico raiz.
        await this.emqxProvisioning.deprovisionRootDeviceUser(device.id).catch((cleanupErr: Error) =>
          this.logger.warn(`Falha ao limpar provisionamento parcial ${device.id}: ${cleanupErr.message}`),
        );
        await this.prisma.device.delete({ where: { id: device.id } }).catch((cleanupErr: Error) =>
          this.logger.warn(`Falha ao remover device MQTT órfão ${device.id}: ${cleanupErr.message}`),
        );
        throw new ServiceUnavailableException(
          'Não foi possível provisionar a credencial MQTT no broker. O dispositivo não foi cadastrado; confira o EMQX e tente novamente.',
        );
      }
    }
  ...
  async updatePoint(
    ...
    if (
      ...
      body.opRole !== undefined ||
      body.siteMetric !== undefined ||
      body.siteMetricMode !== undefined
    ) {
      assertCanEditCriticality(user);
    }

    const data: Prisma.DevicePointUpdateInput = {};
    let effectiveBinding = (point.binding ?? {}) as Record<string, unknown>;
    ...
    // Classificação semântica da visão geral do cliente. A alteração ocorre no
    // mesmo DevicePoint, portanto não toca nas relações de Trend/Telemetry.
    if (body.siteMetric !== undefined || body.siteMetricMode !== undefined) {
      const metricRaw = body.siteMetric !== undefined ? body.siteMetric : point.siteMetric;
      const metric = metricRaw === null || metricRaw === '' ? null : String(metricRaw);
      if (metric === null) {
        data.siteMetric = null;
        data.siteMetricMode = null;
      } else {
        if (!SITE_METRIC_KINDS.has(metric)) {
          throw new BadRequestException('siteMetric deve ser "energy", "water", "runtime" ou null');
        }
        const modeRaw = body.siteMetricMode !== undefined ? body.siteMetricMode : point.siteMetricMode;
        const mode = modeRaw === null || modeRaw === '' ? null : String(modeRaw);
        if (!mode || !SITE_METRIC_MODES.has(mode)) {
          throw new BadRequestException('siteMetricMode deve ser "cumulative", "rate" ou "state"');
        }
        if (metric === 'runtime') {
          if (mode !== 'state') {
            throw new BadRequestException('Pontos de funcionamento devem usar o modo "state"');
          }
          if (!isDigitalPoint(device.protocol, point.objectType, point.binding)) {
            throw new BadRequestException('Horas de funcionamento exigem um ponto digital/booleano');
          }
          const effectiveUnit = body.unit !== undefined ? String(body.unit).trim() : (point.unit ?? '').trim();
          if (effectiveUnit) {
            throw new BadRequestException('Pontos de funcionamento não devem ter unidade analógica');
          }
        } else {
          if (mode !== 'cumulative' && mode !== 'rate') {
            throw new BadRequestException('Medidores de energia/água devem ser acumulativos ou de taxa');
          }
          if (isDigitalPoint(device.protocol, point.objectType, point.binding)) {
            throw new BadRequestException('Medidores de energia/água exigem um ponto numérico');
          }
          const normalizedUnit = normalizeSiteMetricUnit(
            metric,
            mode,
            body.unit !== undefined ? body.unit : point.unit,
          );
          data.unit = normalizedUnit;
        }
        data.siteMetric = metric;
        data.siteMetricMode = mode;
      }
    } else if (body.unit !== undefined && point.siteMetric && point.siteMetricMode) {
      if (point.siteMetric === 'runtime' && String(body.unit).trim()) {
        throw new BadRequestException('Pontos de funcionamento não devem ter unidade analógica');
      }
      if (point.siteMetric === 'energy' || point.siteMetric === 'water') {
        data.unit = normalizeSiteMetricUnit(point.siteMetric, point.siteMetricMode, body.unit);
      }
    }
    ...
    if (body.unit !== undefined) {
      ...
      const effectiveMetric = body.siteMetric !== undefined ? body.siteMetric : point.siteMetric;
      const effectiveMode = body.siteMetricMode !== undefined ? body.siteMetricMode : point.siteMetricMode;
      if ((effectiveMetric === 'energy' || effectiveMetric === 'water') && effectiveMode) {
        unit = normalizeSiteMetricUnit(effectiveMetric, effectiveMode, unit);
      }
      data.unit = unit;
    ...
    // Alterar o tipo técnico de um ponto já classificado não pode deixar uma
    // semântica incompatível persistida.
    const finalMetric = body.siteMetric !== undefined ? body.siteMetric : point.siteMetric;
    const finalMode = body.siteMetricMode !== undefined ? body.siteMetricMode : point.siteMetricMode;
    if (finalMetric && finalMode) {
      const finalIsDigital = isDigitalPoint(device.protocol, point.objectType, effectiveBinding);
      if (finalMetric === 'runtime' && (!finalIsDigital || finalMode !== 'state')) {
        throw new BadRequestException('A edição tornaria inválida a fonte de horas de funcionamento');
      }
      if ((finalMetric === 'energy' || finalMetric === 'water') && finalIsDigital) {
        throw new BadRequestException('A edição tornaria inválida a fonte numérica de consumo');
      }
    }
