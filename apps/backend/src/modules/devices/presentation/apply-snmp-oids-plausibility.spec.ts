  ...
  return {
    device: {
      ...
      findUniqueOrThrow: jest
        .fn()
        .mockResolvedValue({ ...device, points, site: null }),
...
const configPublisher = () => ({
  publishForDevice: jest.fn().mockResolvedValue(undefined),
});
const deviceStatus = () => ({
...
describe('SCA — apply-snmp-oids honra a plausibilidade', () => {
  ...
  it('customPoint reprovado entra como custom neutro, nunca repontando o canônico', async () => {
    ...
    const arg = prisma.devicePoint.create.mock.calls[0][0] as {
  ...
  it('controle: OID confirmado (fora da lista) continua aplicável como canônico', async () => {
    const prisma = buildPrismaMock(
      fakeDevice({ config: { snmpUnconfirmedOids: [] } }),
      [cpuPoint()],
    );
    const controller = buildController(prisma);
  ...
  it('mesmo OID em oids E customPoints repontá o canônico UMA vez, sem ponto duplicado', async () => {
    ...
    for (const call of prisma.devicePoint.update.mock.calls) {
      const data = (call[0] as { data: { binding?: Record<string, unknown> } })
        .data;
...
describe('CFTV — apply-snmp-oids honra a plausibilidade', () => {
  ...
  it('payload direto com OID reprovado como "cpu" NÃO atualiza/cria ponto canônico', async () => {
    const prisma = buildPrismaMock(
      fakeDevice({ monitoredDeviceType: 'CAMERA' }),
      [cpuPoint()],
    );
    const controller = buildController(prisma);

    await expect(
      controller.applySnmpOids(ADMIN_USER, 'dev-1', {
        oids: { cpu: REPROVED_OID },
      }),
    ).rejects.toThrow('não foram confirmadas');

    expect(prisma.devicePoint.update).not.toHaveBeenCalled();
    expect(prisma.devicePoint.create).not.toHaveBeenCalled();
  });

  it('OID reprovado é liberado quando respondeu no job autenticado do diagnóstico', async () => {
    const prisma = buildPrismaMock(
      fakeDevice({ monitoredDeviceType: 'CAMERA' }),
      [cpuPoint()],
    );
    prisma.snmpDiagnoseJob.findUnique.mockResolvedValue({
      deviceId: 'dev-1',
      tenantId: 'tenant-1',
      status: 'done',
      result: {
        metrics: [
          {
            candidates: [{ oid: REPROVED_OID, responded: true }],
          },
        ],
      },
    });
    ...
    await controller.applySnmpOids(ADMIN_USER, 'dev-1', {
      diagnoseId: 'diagnose-1',
      oids: { cpu: REPROVED_OID },
    });

    expect(prisma.devicePoint.update).toHaveBeenCalledWith(
      expect.objectContaining({ where: { id: 'pt-cpu' } }),
    );
  });
