...
} from '../application/onvif-probe.service.js';
import {
...
/** Escalas das fontes de RAM total para o contrato canônico em bytes. */
const CANONICAL_RAM_TOTAL_SCALES = new Map<string, number>([
  ['1.3.6.1.4.1.39165.1.10.0', 1024 * 1024],
  ['1.3.6.1.4.1.50001.1.220.0', 1024 * 1024],
  ['1.3.6.1.2.1.25.2.2.0', 1024],
  ['1.3.6.1.4.1.2021.4.5.0', 1024],
]);

function canonicalMetricScale(metric: string, oid: string): number {
  return normalizeMetricKey(metric) === 'ram_total'
    ? (CANONICAL_RAM_TOTAL_SCALES.get(oid) ?? 1)
    : 1;
}
...
const DEFAULT_ONVIF_POINTS = [
  ...
  {
    tag: 'STREAM',
    objectName: 'Stream de vídeo (disponível)',
    metric: 'stream',
    unit: '',
  },
  {
    tag: 'MOVIMENTO',
    objectName: 'Detecção de movimento',
    metric: 'motion',
    unit: '',
  },
  ...
  {
    tag: 'PERDA_VIDEO',
    objectName: 'Perda de vídeo',
    metric: 'video_loss',
    unit: '',
  },
  {
    tag: 'LATENCIA',
    objectName: 'Latência de resposta',
    metric: 'latency',
    unit: 'ms',
  },
  {
...
export class CftvController {
  ...
  /** Evita duas descobertas automáticas concorrentes do mesmo equipamento. */
  private readonly autoDiscoveryInFlight = new Set<string>();
  ...
  async keepAliveLiveView(
    ...
    return this.liveView.keepAlive(
      { id: user.id, tenantId: user.tenantId },
      sessionId,
    );
  }
  ...
  private async assertGatewayInTenant(
    gatewayId: string,
    tenantId: string,
  ): Promise<void> {
  ...
  async createCamera(
    ...
    if (!body.gatewayId) {
      throw new BadRequestException(
        'gatewayId é obrigatório (faz o monitoramento)',
      );
    }
    ...
    const protocol =
      body.monitoringProtocol === 'onvif' ? ONVIF_PROTOCOL : SNMP_PROTOCOL;
    ...
    const snmpVersion: SnmpVersion =
      body.snmpVersion === '1' || body.snmpVersion === '3'
        ? body.snmpVersion
        : '2c';
    ...
    const selectedProfileId =
      rawProfileId === null ||
      rawProfileId === GENERIC_PROFILE.id ||
      rawProfileId === undefined
        ? null
        : rawProfileId;
    if (
      selectedProfileId &&
      !CAMERA_OID_PROFILES.some((profile) => profile.id === selectedProfileId)
    ) {
      throw new BadRequestException(
        `ID de perfil desconhecido: "${selectedProfileId}". ` +
          `Perfis válidos: ${CAMERA_OID_PROFILES.map((profile) => profile.id).join(', ')}.`,
      );
    }
    const selectedProfile = selectedProfileId
      ? (CAMERA_OID_PROFILES.find(
          (profile) => profile.id === selectedProfileId,
        ) ?? GENERIC_PROFILE)
      : GENERIC_PROFILE;
    const manualProfileSelected = selectedProfileId !== null;
    const manualOids = {
      ...(body.profileOverrides ?? {}),
      ...(body.healthOids ?? {}),
    };
    const initialPoints = this.buildInitialSnmpCameraPoints(
      selectedProfile,
        ...
        config: {
          ...
          ...(body.profileId !== undefined
            ? {
                ...
                profileSource: selectedProfileId ? 'manual' : 'generic',
              }
            : {}),
          ...(body.profileOverrides &&
          Object.keys(body.profileOverrides).length > 0
            ? {
    ...
    this.logger.log(
      `Câmera CFTV cadastrada: ${camera.id} (${ip}) por ${user.email}`,
    );

    // Descoberta de cadastro fire-and-forget: além de registrar o walk, aplica
    // as fontes comprovadas, republica o plano efetivo e semeia a primeira
    // leitura. Automática = no máx. 1×/dia por device.
    void this.finishCameraRegistrationSetup(camera.id).catch((err: Error) => {
      this.logger.warn(
        `Configuração pós-cadastro da câmera ${camera.id} falhou (não bloqueante): ${err.message}`,
      );
    });
  ...
  private async createOnvifCamera(
    ...
    return this.mapCamera(
      created,
      await this.deviceStatus.resolveLastSeen(created.id),
    );
  }
  ...
  async updateCamera(
    ...
    if (body.siteId) {
      ...
      if (!site)
        throw new BadRequestException('Site inválido para esta câmera');
    }
    ...
    if (!isOnvif) {
      ...
      if (
        ...
        const mergedBody: SnmpCredentialBody = {
          ...
          community:
            ...
            'public',
          securityName:
            body.securityName?.trim() || existing?.securityName || undefined,
          authProtocol:
            body.authProtocol ?? existing?.authProtocol ?? undefined,
          privProtocol:
            body.privProtocol ?? existing?.privProtocol ?? undefined,
          contextName: body.contextName ?? existing?.contextName ?? undefined,
        };
        const credentialData = buildSnmpCredentialData(
          nextVersion,
          mergedBody,
          existing,
        );
      ...
      if (connectionChanged || cfg.pendingValidation === true) {
        ...
        const password =
          newPassword ||
          decryptCameraSecret(cfg.onvifPasswordEnc as string | undefined);
        const username =
          newUsername || (cfg.onvifUsername as string | undefined);
        if (!username || !password) {
          throw new BadRequestException(
            'Usuário e senha ONVIF são obrigatórios',
          );
        }
        const probe = await this.onvifProbe.probe({
          tenantId: camera.tenantId,
        ...
        if (probe.success) {
          ...
          newConfig = {
            ...
            onvifUsername: username,
            ...(newPassword
              ? { onvifPasswordEnc: encryptCameraSecret(newPassword) }
              : {}),
    ...
    if (body.profileId !== undefined) {
      ...
      const rawProfileId = body.profileId;
      const newProfileId =
        rawProfileId === null || rawProfileId === GENERIC_PROFILE.id
          ? null
          : rawProfileId;
    ...
    return this.mapCamera(
      updated,
      await this.deviceStatus.resolveLastSeen(updated.id),
    );
  }
  ...
  async deleteCamera(
    ...
    if (user.role !== 'ADMIN' && user.role !== 'CCO') {
      throw new BadRequestException(
        'Apenas ADMIN ou CCO podem excluir câmeras',
      );
    }
    const camera = await this.findCameraOrThrow(id);
    ...
    if (camera.gatewayId) {
      await this.configPublisher.publishForGateway(
        camera.tenantId,
        camera.gatewayId,
      );
    }
  ...
  async deleteCameraPoint(
    ...
    if (user.role !== 'ADMIN' && user.role !== 'CCO') {
      throw new BadRequestException(
        'Apenas ADMIN ou CCO podem remover pontos de câmeras',
      );
    }
    const camera = await this.findCameraOrThrow(id);
    this.assertCanEdit(user, camera.tenantId);
    ...
    if (!point)
      throw new NotFoundException('Ponto não encontrado nesta câmera');
  ...
  /**
   * POST /cftv/cameras/:id/diagnose-snmp — dispara o diagnóstico do canal
   * SNMP da câmera via gateway (testa cada OID cadastrado e os candidatos de
   * TODOS os perfis de fabricante, e faz um walk resumido) e responde de
   * imediato confirmando o início — o proxy de produção corta requests
   * >30s e o gateway pode levar até ~2min. A execução completa (chamada ao
   * gateway + enriquecimento) roda em segundo plano; o resultado final é
   * consultado via GET /cftv/diagnose/:diagnoseId/progress (polling). Falha
   * rápido (síncrono) se o gateway está offline ou a câmera está mal configurada.
   */
  ...
  async diagnoseSnmp(
    ...
    const currentByMetric: Record<
      string,
      { oid: string; pointId: string; scale: number }
    > = {};
    ...
    // "pending" para sempre.
    void this.runCameraSnmpDiagnoseJob({
      diagnoseId,
      camera,
      port,
      snmpVersion,
      ...
      this.logger.error(
        `Diagnóstico SNMP ${diagnoseId} (câmera ${id}) falhou de forma inesperada: ${(err as Error).message}`,
      );
  ...
  /**
   * Executa o diagnóstico SNMP da câmera (chamada ao gateway + todo o
   ...
   */
  private async runCameraSnmpDiagnoseJob(params: {
    diagnoseId: string;
    camera: Awaited<ReturnType<CftvController['findCameraOrThrow']>>;
    port: number;
    snmpVersion: '1' | '2c' | '3';
    community: string;
    v3: ReturnType<typeof resolveSnmpRuntimeCredentials>['v3'];
    current: DiagnoseOidProbe[];
    currentByMetric: Record<
      string,
      { oid: string; pointId: string; scale: number }
    >;
    candidates: DiagnoseOidProbe[];
    catalog: ReturnType<typeof buildDiagnoseCandidateCatalog>;
    ...
    const metrics = DIAG_METRICS.map((metric) => {
      ...
      const supported =
        Boolean(curRead?.responded) ||
        metricCandidates.some((c) => c.responded);
      return {
        metric,
        ...
        currentValue:
          curRead?.value != null ? curRead.value * (cur?.scale ?? 1) : null,
    ...
    if (result.reachable) {
      for (const metric of DIAG_METRICS) {
        ...
        if (
          Boolean(b.unsupported) !== unsupported ||
          b.healthState !== healthState
        ) {
          await this.prisma.devicePoint.update({
            where: { id: point.id },
            data: {
    ...
    let discovery: Awaited<
      ReturnType<SnmpDiscoveryPersistenceService['recordRun']>
    > | null = null;
    try {
    ...
    if (result.reachable) {
      try {
        ...
        const resolvedFromCanonical = Object.values(
          result.canonicalMetrics ?? {},
        )
          ...
          .map((cm) => {
            ...
            return {
              ...
              oid,
              scale: canonicalMetricScale(cm.canonicalKey, oid),
              unit: cm.unit,
              memberOids: [
                ...new Set([
                  ...(cm.memberOids ?? []),
        ...
        const canonicalKeys = new Set(
          resolvedFromCanonical.map((r) => normalizeMetricKey(r.metricKey)),
        );
        ...
        const existingBindings =
          await this.snmpMetric.getBindingsForProposals(id);
        const currentOidsByMetric: Record<string, string> = {};
        for (const m of metrics) {
          if (m.currentOid)
            currentOidsByMetric[normalizeMetricKey(m.metric)] = m.currentOid;
        }
        ...
        proposals = this.snmpMetric.buildProposals({
          ...
          diagnoseResult: {
            ...result,
            canonicalMetrics: result.canonicalMetrics,
          },
  ...
  private async runAutoDiscovery(
    ...
    if (this.autoDiscoveryInFlight.has(deviceId)) return;
    this.autoDiscoveryInFlight.add(deviceId);
    try {
      await this.runAutoDiscoveryOnce(deviceId, trigger);
    } finally {
      this.autoDiscoveryInFlight.delete(deviceId);
    }
  }

  private async runAutoDiscoveryOnce(
    deviceId: string,
    trigger: 'registration' | 'scheduled',
  ): Promise<void> {
    ...
    if (result.reachable && result.canonicalMetrics) {
      // O diagnóstico pode levar segundos; recarrega antes de aplicar para que
      // uma escolha manual feita enquanto ele rodava sempre tenha precedência.
      const currentDevice = await this.prisma.device.findUnique({
        where: { id: deviceId },
      });
      if (!currentDevice) return;
      await this.reconcileAutoDiscoveredCameraMetrics(currentDevice, result);
  ...
  /**
   * Mantém o plano de coleta da câmera alinhado ao diagnóstico de cadastro,
   * sem recriar pontos. Um ponto que já passou pelo apply manual carrega
   * `unsupported` (mesmo quando false) e não é alterado aqui.
   */
  private async reconcileAutoDiscoveredCameraMetrics(
    device: {
      id: string;
      tenantId: string;
      config: unknown;
    ...
    result: Extract<
      Awaited<ReturnType<SnmpDiagnoseService['diagnose']>>,
      { success: true }
    >,
  ): Promise<void> {
    const canonical = Object.values(result.canonicalMetrics ?? {})
      .filter(
        (metric) =>
          metric.value !== null &&
      ...
      .map((metric) => ({
        key: normalizeMetricKey(metric.canonicalKey),
        oid:
          metric.selectedOid ??
          metric.memberOids?.[0] ??
          metric.dependencyOids?.[0] ??
          null,
        unit: metric.unit,
        scale: canonicalMetricScale(
          metric.canonicalKey,
          metric.selectedOid ??
            metric.memberOids?.[0] ??
            metric.dependencyOids?.[0] ??
    ...
    let nextInstance =
      points.reduce((max, point) => Math.max(max, point.instance), -1) + 1;
    for (const metric of canonical) {
      const existing = points.find((point) => {
        const binding = (point.binding ?? {}) as { metric?: string };
        return normalizeMetricKey(binding.metric ?? point.tag) === metric.key;
      });
      const isManuallyApplied = existing
        ? Object.prototype.hasOwnProperty.call(
            (existing.binding ?? {}) as Record<string, unknown>,
            'unsupported',
          )
        : false;
      if (isManuallyApplied) continue;

      const meta = this.cameraMetricMeta(metric.key);
      const binding = {
        metric: metric.key,
        oid: metric.oid,
        scale: metric.scale,
        ...(metric.memberOids.length > 0
          ? { memberOids: metric.memberOids }
          : {}),
      };
      if (existing) {
        // Compare-and-set do JSON completo: se o operador aplicou uma fonte
        // manual depois do snapshot, o binding já mudou e não o sobrescrevemos.
        const updated = await this.prisma.devicePoint.updateMany({
          where: {
            id: existing.id,
            binding: { equals: existing.binding as Prisma.InputJsonValue },
          },
          data: { objectName: meta.objectName, unit: metric.unit, binding },
        });
    ...
    // Perfil detectado é só uma sugestão persistida. Uma seleção explícita do
    // operador permanece soberana e o polling continua governado pelos bindings.
    const detectedProfileId = canonical.some((metric) =>
      metric.oid.startsWith('1.3.6.1.4.1.39165.'),
    )
      ? 'hikvision'
      : null;
    const cfg = (device.config ?? {}) as Record<string, unknown>;
    if (detectedProfileId && cfg.profileSource !== 'manual') {
      // Não substitui uma seleção manual feita enquanto a descoberta aplicava
      // os pontos. A igualdade do snapshot funciona como compare-and-set.
      await this.prisma.device.updateMany({
        where: {
          id: device.id,
          config: { equals: cfg as Prisma.InputJsonValue },
        },
        data: {
          config: {
            ...cfg,
    ...
    // A tabela de bindings guarda a fonte e os membros de agregados. O publish
    // logo abaixo espelha também os pontos preservados/criados no mesmo plano.
    await this.snmpMetric.persistAutoResolvedBindings({
      tenantId: device.tenantId,
      deviceId: device.id,
      sysObjectId: result.sysObjectId,
      firmwareFamily: extractFirmwareFamily({
        walk: result.walk,
        sysDescr: result.sysDescr,
      }),
      resolved: canonical.map((metric) => ({
  ...
  private cameraMetricMeta(metricKey: string): {
    tag: string;
    objectName: string;
  } {
    switch (metricKey) {
      case 'cpu_usage':
        return { tag: 'CPU', objectName: 'Uso de CPU' };
      case 'memory_used_percent':
        return { tag: 'MEMORIA', objectName: 'Memória usada' };
      case 'ram_total':
        return { tag: 'RAM_TOTAL', objectName: 'Memória RAM total' };
      case 'storage_used_percent':
        return { tag: 'ARMAZENAMENTO', objectName: 'Uso de armazenamento' };
      default:
        return { tag: metricKey.toUpperCase(), objectName: metricKey };
    }
  }

  private buildInitialSnmpCameraPoints(
    profile: CameraOidProfile,
    manualProfileSelected: boolean,
    manualOids: Record<string, string | undefined>,
  ) {
    const definitions: Array<{
      ...
      unit: string;
      unsupported?: boolean;
    }> = DEFAULT_CAMERA_POINTS.map((point) => {
      const metric = point.metric as HealthMetric;
      const override = manualOids[metric]?.trim();
      const profileEntry = profile.oids[metric] ?? null;
      const isManual = manualProfileSelected || Boolean(override);
      return {
        ...point,
        oid: override || profileEntry?.oid || point.oid,
        scale: override ? 1 : (profileEntry?.scale ?? point.scale),
        unit: override
          ? (profileEntry?.unit ?? point.unit)
          : (profileEntry?.unit ?? point.unit),
        ...(isManual ? { unsupported: false } : {}),
      };
    });

    // Perfis homologados podem expor métricas que não existem no seed genérico
    // (Hikvision: RAM total e armazenamento). Criamos esses pontos desde o
    // cadastro, sem depender de uma descoberta posterior.
    for (const metric of ['ram_total', 'storage'] as const) {
      const profileEntry = profile.oids[metric];
      const override = manualOids[metric]?.trim();
      if (!profileEntry && !override) continue;
      const meta = HEALTH_METRIC_META[metric];
      definitions.push({
        tag: meta.tag,
        objectName: meta.objectName,
        metric,
        oid: override || profileEntry?.oid || null,
        scale: override ? 1 : (profileEntry?.scale ?? 1),
        unit: override
          ? (profileEntry?.unit ?? '')
          : (profileEntry?.unit ?? ''),
        ...(manualProfileSelected || override ? { unsupported: false } : {}),
      });
    }
    return definitions;
  }

  private async finishCameraRegistrationSetup(deviceId: string): Promise<void> {
    await this.runAutoDiscovery(deviceId, 'registration');
    const device = await this.prisma.device.findUnique({
      where: { id: deviceId },
      include: { points: true, snmpCredential: true },
    });
    if (device) await this.seedCameraFromEffectivePlan(device);
  }

  /**
   * Usa os OIDs efetivos já persistidos nos pontos, não o catálogo genérico.
   * Assim seed, diagnóstico e polling sempre falam da mesma fonte homologada.
   */
  private async seedCameraFromEffectivePlan(
    device: Prisma.DeviceGetPayload<{ include: { points: true } }>,
  ): Promise<void> {
    if (
      !device.gatewayId ||
      this.deviceStatus.getStatus(device.gatewayId) === 'offline'
    )
      return;
    const cfg = (device.config ?? {}) as {
      snmpVersion?: string;
    ...
    const creds = resolveSnmpRuntimeCredentials(credential, cfg);
    // A chave é o próprio ponto: duas fontes da mesma métrica jamais podem
    // trocar valores entre si durante o seed, e um ponto unsupported não entra
    // nem na requisição nem no write.
    const oids: Record<string, string> = {};
    const seedPlan = new Map<string, { pointId: string; scale: number }>();
    for (const point of device.points) {
      ...
      const metric = binding.metric ? normalizeMetricKey(binding.metric) : null;
      if (!metric || !binding.oid || binding.unsupported) continue;
      oids[point.id] = binding.oid;
      seedPlan.set(point.id, { pointId: point.id, scale: binding.scale ?? 1 });
    }
    if (Object.keys(oids).length === 0) return;
    ...
    const at = new Date();
    for (const point of device.points) {
      const binding = (point.binding ?? {}) as { unsupported?: boolean };
      if (binding.unsupported) continue;
      const plan = seedPlan.get(point.id);
      if (!plan) continue;
  ...
  /**
   * GET /cftv/diagnose/:diagnoseId/progress — status do job de diagnóstico
  ...
  async applySnmpOids(
    ...
    body: {
       ...
       */
      metricConfidence?: Partial<
        Record<string, 'exact' | 'inferred' | 'manual'>
      >;
      /**
       * Diagnóstico que produziu as sugestões exibidas no modal. O backend
       * valida este job antes de liberar um OID que o último walk marcou como
       * semanticamente não confirmado.
       */
      diagnoseId?: string;
    },
  ) {
    ...
    const entries = Object.entries(body?.oids ?? {}).flatMap(
      ([metric, raw]) => {
        const isLegacy = DIAG_METRICS.includes(metric as DiagMetric);
        const isCanonical =
          CANONICAL_METRIC_KEYS.has(metric) && !METRICS_WITHOUT_OID.has(metric);
        const selection = normalizeSnmpOidSelection(raw);
        return isLegacy || isCanonical
          ? selection
            ? [[metric, selection] as const]
            : []
          : [];
      },
    );
      ...
      if (
        ...
        for (const volume of storageVolumes) {
          ...
          if (existingVolume) {
            ...
            const nextInstance =
              points.reduce(
                (max, candidate) => Math.max(max, candidate.instance),
                -1,
              ) + 1;
      ...
      if (!point) {
        ...
        const meta = (
          ...
          (
            ...
            tag: metric.toUpperCase().replace(/_/g, ''),
            objectName: metric,
          };
        const nextInstance =
          points.reduce((m, p) => Math.max(m, p.instance), -1) + 1;
        const created = await this.prisma.devicePoint.create({
          data: {
            deviceId: id,
            tag: meta.tag,
    ...
    if (
      ...
      throw new BadRequestException(
        'As leituras selecionadas não foram confirmadas pelo diagnóstico mais recente. Execute um novo diagnóstico e tente novamente.',
      );
    }

    // Um OID liberado por um diagnóstico autenticado deixa de ser "não
    // confirmado" para os próximos ciclos. Sem esta limpeza, a câmera voltava
    // a rejeitar a mesma fonte na próxima tentativa de ativação.
    if (acceptedOids.size > 0 && unconfirmedOids.size > 0) {
      const remainingUnconfirmed = [...unconfirmedOids].filter(
        (oid) => !acceptedOids.has(oid),
      );
      if (remainingUnconfirmed.length !== unconfirmedOids.size) {
        const currentCfg = (camera.config ?? {}) as Record<string, unknown>;
        await this.prisma.device.update({
          where: { id },
          data: {
            config: {
              ...currentCfg,
              snmpUnconfirmedOids: remainingUnconfirmed,
            } as unknown as Prisma.InputJsonValue,
          },
        });
      }
    ...
    for (const [metric, selection] of entries) {
      ...
      try {
        await this.snmpMetric.persistBinding({
          ...
          unit,
          confidence:
            body?.metricConfidence?.[metric] ??
            body?.metricConfidence?.[normalizedMetric],
        });
  ...
  /**
   * Retorna somente OIDs que responderam no job de diagnóstico fornecido pelo
   * próprio modal. A lista evita que um payload forjado use um OID marcado como
   * semanticamente não confirmado, mas permite ativar uma fonte vendor-specific
   * que o probe confirmou agora (como CPU/memória de Hikvision).
   */
  private async getVerifiedDiagnosticOids(
    ...
    const job = await this.prisma.snmpDiagnoseJob.findUnique({
      where: { id },
      select: { deviceId: true, tenantId: true, status: true, result: true },
    });
    if (
      !job ||
    ...
    const result = (job.result ?? {}) as unknown as {
      metrics?: Array<{
        candidates?: Array<{ oid?: unknown; responded?: unknown }>;
      }>;
    };
    const verified = new Set<string>();
    for (const metric of result.metrics ?? []) {
      for (const candidate of metric.candidates ?? []) {
        if (candidate.responded === true && typeof candidate.oid === 'string') {
          verified.add(candidate.oid);
        }
      }
    }
    return verified;
  ...
  async listSwitches(
    ...
    return switches.map((sw) =>
      this.mapSwitch(
        sw as Parameters<typeof this.mapSwitch>[0],
        lastSeenMap.get(sw.id) ?? null,
      ),
    );
  }
      ...
      data: {
        ...
        config: {
          ...
          ...(body.profileId !== undefined
            ...
            : {}),
          ...(body.profileOverrides !== undefined
            ? { profileOverrides: body.profileOverrides }
            : {}),
        },
        points: {
  ...
  async getSwitch(
    ...
    return this.mapSwitch(
      withPoints as Parameters<typeof this.mapSwitch>[0],
      lastCommunication,
    );
  }
  ...
  async updateSwitch(
    ...
    const updatedConfig = {
      ...
      ...(body.pollingInterval !== undefined
        ? { pollingIntervalMs: body.pollingInterval * 1000 }
        : {}),
      ...(body.manufacturer !== undefined
        ? { manufacturer: body.manufacturer?.trim() || null }
        : {}),
    ...
    await this.configPublisher.publishForDevice(id);
    return this.mapSwitch(
      updated as Parameters<typeof this.mapSwitch>[0],
      await this.deviceStatus.resolveLastSeen(id),
    );
  }
  ...
  async syncPorts(
    ...
    if (!gwOnline) {
      throw new BadRequestException(
        'Gateway offline — não é possível descobrir portas agora',
      );
    }
    ...
    const { visible: ports, creatable } = partitionDiscoveredPorts(
      discoverResult.ports,
    );
    ...
    const existingByIndex = new Map<
      number,
      Array<(typeof existingPortPoints)[0]>
    >();
    ...
    const removed = [...existingIndexes].filter(
      (idx) => !discoveredIndexes.has(idx),
    );
    ...
    for (const port of toAdd) {
      const portLabel =
        port.ifAlias?.trim() || port.ifDescr?.trim() || `Porta ${port.ifIndex}`;
    ...
    for (const port of toCheck) {
      ...
      for (const p of existingPoints) {
        ...
        if (p.objectType === 'sw-state')
          expectedName = `${portLabel}${speedSuffix} — Status`;
        else if (p.objectType === 'sw-in')
          expectedName = `${portLabel}${speedSuffix} — Tráfego entrada`;
  ...
  async deletePort(
    ...
    await this.configPublisher.publishForDevice(id);
    this.logger.log(
      `Switch ${id} porta ifIndex=${ifIndex} removida por ${user.email}`,
    );
  }
  ...
  async listNvrs(
    ...
    return nvrs.map((nvr) =>
      this.mapNvr(
        nvr as Parameters<typeof this.mapNvr>[0],
        lastSeenMap.get(nvr.id) ?? null,
      ),
    );
  }
  ...
  async getNvr(
    ...
    const lastCommunication = await this.deviceStatus.resolveLastSeen(id);
    return this.mapNvr(
      withPoints as Parameters<typeof this.mapNvr>[0],
      lastCommunication,
    );
  }
  ...
  async updateNvr(
    ...
    await this.configPublisher.publishForDevice(id);
    return this.mapNvr(
      updated as Parameters<typeof this.mapNvr>[0],
      await this.deviceStatus.resolveLastSeen(id),
    );
  }
  ...
  async syncNvrDisks(
    ...
    if (!gwOnline) {
      throw new BadRequestException(
        'Gateway offline — não é possível descobrir discos agora',
      );
    }
    ...
    let resolvedProfileId: string | null = hasVendorOids(cfg.profileId)
      ? cfg.profileId
      : null;
    ...
    if (!resolvedProfileId && discoverResult.sysDescr) {
      const detected = detectNvrProfile(
        discoverResult.sysDescr,
        null,
    ...
    const existingDiskPoints = await this.prisma.devicePoint.findMany({
      where: {
        deviceId: id,
        objectType: { in: ['nvr-disk', 'nvr-disk-cap', 'nvr-disk-used'] },
      },
    });
    ...
    const diskBySlot = new Map<
      number,
      {
        status?: (typeof existingDiskPoints)[0];
        cap?: (typeof existingDiskPoints)[0];
        used?: (typeof existingDiskPoints)[0];
      }
    >();
    ...
    for (const disk of discoveredDisks) {
      ...
      const scaledCapacity: number | null =
        capacityValue !== null ? capacityValue * diskScale : null;
      const scaledFree: number | null =
        freeValue !== null ? freeValue * diskScale : null;
              ...
              create: [
                {
                  ...
                  ...(status !== null
                    ? { lastValue: status, lastValueAt: now }
                    : {}),
                },
                {
                  ...
                  ...(scaledCapacity !== null
                    ? { lastValue: scaledCapacity, lastValueAt: now }
                    : {}),
                },
                {
                  ...
                  binding: {
                    ...
                    ...(!usedIsPercent && diskScale !== 1
                      ? { scale: diskScale }
                      : {}),
                  },
                  ...(normalizedUsed !== null
                    ? { lastValue: normalizedUsed, lastValueAt: now }
                    : {}),
                },
              ],
            },
    ...
    return {
      ...
      disks: discoveredDisks.map((d) => {
        ...
        const capacityGb =
          d.capacityValue !== null
            ? d.capacityValue * diskScaleForResponse
            : null;
        const usedGb =
          rawUsed !== null
            ? usedIsPercent
              ? rawUsed
              : rawUsed * diskScaleForResponse
            : null;
        return {
          ...
          statusLabel:
            d.status !== null
              ? (diskStatusLabels[d.status] ?? String(d.status))
              : null,
          capacityGb,
          usedGb,
      ...
      channels: discoveredChannels.map((c) => ({
        ...
        statusLabel:
          c.status !== null
            ? (chanStatusLabels[c.status] ?? String(c.status))
            : null,
      })),
    };
  }
  ...
  private mapNvr(
    ...
    const scalarPoints = nvr.points.filter(
      (p) =>
        !['nvr-disk', 'nvr-disk-cap', 'nvr-disk-used', 'nvr-chan'].includes(
          p.objectType ?? '',
        ),
    );
    const diskPoints = nvr.points.filter((p) =>
      ['nvr-disk', 'nvr-disk-cap', 'nvr-disk-used'].includes(
        p.objectType ?? '',
      ),
    );
    ...
    return {
      ...
      snmpInfo: ((nvr.config ?? {}) as { snmpInfo?: unknown }).snmpInfo as
        SnmpInfoEntry[] | undefined,
        ...
        .map((entry) => ({
          ...
          statusPoint: entry.statusPoint
            ? {
                ...
                statusLabel:
                  entry.statusPoint.lastValue !== null
                    ? (diskStatusLabels[Number(entry.statusPoint.lastValue)] ??
                      String(entry.statusPoint.lastValue))
                    : null,
              }
            : null,
  ...
  private mapSwitch(
    ...
    const scalarPoints = sw.points.filter(
      (p) => !['sw-state', 'sw-in', 'sw-out'].includes(p.objectType ?? ''),
    );
    const portPoints = sw.points.filter((p) =>
      ['sw-state', 'sw-in', 'sw-out'].includes(p.objectType ?? ''),
    );
    ...
    for (const p of portPoints) {
      const ifIndex = p.instance;
      if (ifIndex === null) continue;
      const entry = portsByIndex.get(ifIndex) ?? {
        ifIndex,
        statePoint: null,
        inPoint: null,
        outPoint: null,
    ...
    return {
      ...
      snmpInfo: ((sw.config ?? {}) as { snmpInfo?: unknown }).snmpInfo as
        SnmpInfoEntry[] | undefined,
        ...
        .map((entry) => ({
          ...
          statePoint: entry.statePoint
            ? {
                ...
                lastValueAt:
                  entry.statePoint.lastValueAt?.toISOString() ?? null,
              }
            : null,
  ...
  private expandOnvifTargets(input?: string): string[] {
    ...
    for (const piece of raw
      ...
      if (cidr) {
        ...
        const base =
          toNum(cidr[1]) & (prefix === 0 ? 0 : (~0 << (32 - prefix)) >>> 0);
  ...
  private mapCamera(
    ...
    for (const p of camera.points) {
      ...
      if (
        b.metric &&
        b.oid &&
        (HEALTH_METRICS as string[]).includes(b.metric)
      ) {
    ...
    return {
      ...
      liveViewAvailable: Boolean(
        (typeof cfg.onvifUsername === 'string' &&
          cfg.onvifUsername.trim() &&
          typeof cfg.onvifPasswordEnc === 'string' &&
          cfg.onvifPasswordEnc) ||
      ...
      points: camera.points.map((p) => {
        ...
        return {
          ...
          unsupported: Boolean(b.unsupported),
          healthState:
            b.healthState === 'pending' &&
            p.lastValueAt &&
            p.lastValueState === null
              ? 'active'
              : b.healthState,
