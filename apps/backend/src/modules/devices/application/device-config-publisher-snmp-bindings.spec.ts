...
function fakePoint(tag: string, binding: Record<string, unknown> | null) {
  return { id: `point-${tag}`, tag, objectType: 'AV', instance: 0, unit: null, binding };
}
...
describe('DeviceConfigPublisherService — coleta governada por device_metric_binding', () => {
  ...
  it('publica RAM Hikvision em bytes com a escala MB→bytes no plano restrito', async () => {
    const hikRamTotal = '1.3.6.1.4.1.39165.1.10.0';
    const device = fakeSnmpDevice({
      config: { snmpVersion: '2c', community: 'public', manufacturer: 'Hikvision' },
      points: [
        fakePoint('STATUS', { metric: 'status' }),
        {
          ...fakePoint('RAM_TOTAL', {
          ...
          // Plano legado: a unidade do ponto ainda descreve a fonte em MB.
          unit: 'MB',
        },
      ],
    });
    ...
    expect(firstBlock(published).points.find((point) => point.tag === 'RAM_TOTAL')).toMatchObject({
      metric: 'ram_total',
      oid: hikRamTotal,
      unit: 'bytes',
      scale: 1024 * 1024,
