...
const KB_RAM_TOTAL_OIDS = new Set([
...
/**
 * Compila a escala do ponto para a unidade canônica publicada pelo gateway.
 * Os OIDs conhecidos protegem planos antigos que já tiveram sua unidade
 * atualizada para bytes, mas ainda guardam scale=1.
 */
function canonicalSnmpScale(
  ...
  if (baseScale !== 1) return baseScale;
  if (metric === 'ram_total' && oid && HIKVISION_RAM_TOTAL_OIDS.has(oid)) {
    return 1024 * 1024;
  }
  if (metric === 'ram_total' && oid && KB_RAM_TOTAL_OIDS.has(oid)) {
    return 1024;
  }
...
export class DeviceConfigPublisherService implements OnApplicationBootstrap {
  ...
  async publishForGateway(tenantId: string, gatewayId: string): Promise<void> {
    ...
    // Migração preguiçosa, idempotente e restrita ao OID Hikvision: os planos
    // antigos já publicavam telemetria canônica após a normalização do payload,
    // mas o DevicePoint ainda dizia "MB", o que faria o card interpretar bytes
    // como megabytes. Atualizamos ponto e binding antes do primeiro publish.
    await this.canonicalizeLegacyHikvisionRamPoints(devices);
  ...
  private async canonicalizeLegacyHikvisionRamPoints(devices: Array<{
    protocol: string;
    points: Array<{
      id: string;
      tag: string;
    ...
    for (const device of devices) {
      ...
      for (const point of device.points) {
        const binding = (point.binding ?? {}) as SnmpBinding;
        const metric = normalizeMetricKey(binding.metric ?? point.tag);
        if (
          metric !== 'ram_total' ||
          !binding.oid ||
          !HIKVISION_RAM_TOTAL_OIDS.has(binding.oid)
        ) {
          continue;
        }

        const scale = canonicalSnmpScale(metric, binding.oid, point.unit, binding.scale);
        const alreadyCanonical =
          point.unit?.trim().toLowerCase() === 'bytes' &&
          binding.scale === scale;
        if (alreadyCanonical) continue;

        const nextBinding = { ...binding, metric, scale };
        await this.prisma.devicePoint.update({
          where: { id: point.id },
          data: {
            unit: 'bytes',
            binding: nextBinding as Prisma.InputJsonValue,
          },
        });
        // O bloco desta mesma publicação precisa refletir o estado que acabou
        // de ser persistido; não espera uma segunda publicação/reload.
        point.unit = 'bytes';
        point.binding = nextBinding;
      }
    }
  }
    ...
    if (device.protocol === 'snmp') {
      ...
      const points = device.points.map((p) => {
        ...
        return {
          ...
          scale: canonicalSnmpScale(
            b.metric,
            isTable ? tableOid : scalarOid,
            p.unit ?? null,
            b.scale,
          ),
