...
interface AddScadaProjectDto {
  projectId: string;
  /** Data URL transitório; os bytes são enviados ao App Storage no backend. */
  coverImageDataUrl?: string;
}

interface UpdateScadaProjectCoverDto {
  /** Data URL base64; null remove a capa; omitido não é aceito neste PATCH. */
...
export class ScadaProjectsController {
  ...
  async add(
    ...
    return this.scadaService.enableProject(body.projectId, tenantId, body.coverImageDataUrl);
  }

  @Patch('/:projectId')
  async updateCover(
    @Param('projectId') projectId: string,
    @Body() body: UpdateScadaProjectCoverDto,
    @CurrentUser() user: AuthenticatedUser,
  ): Promise<ScadaProject> {
    if (!GLOBAL_ROLES.has(user.role)) {
      throw new ForbiddenException('Sem permissão para editar projetos SCADA');
    }
    if (
      !body ||
      !Object.prototype.hasOwnProperty.call(body, 'coverImageDataUrl') ||
      (body.coverImageDataUrl !== null && typeof body.coverImageDataUrl !== 'string')
    ) {
      throw new BadRequestException('coverImageDataUrl deve ser uma data URL ou null');
    }
    const tenantId = resolveTenantScope(user);
    return this.scadaService.updateProjectCover(projectId, tenantId, body.coverImageDataUrl);
  }
