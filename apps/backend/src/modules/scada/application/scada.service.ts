...
export class ScadaService {
  ...
  async findAll(filters: ScadaScreenFilters): Promise<ScadaScreen[]> {
    return this.prisma.scadaScreen.findMany({
      ...
      // A tela mantém sua posição original depois de ser editada. O id
      // desempata telas criadas no mesmo instante de forma determinística.
  ...
  async findEnabledProjects(tenantId: string | undefined): Promise<ScadaProject[]> {
    return this.prisma.project.findMany({
      ...
      orderBy: { name: 'asc' },
      select: { id: true, name: true, siteId: true, tenantId: true, coverImageUrl: true },
    });
  }

  /**
   * Adiciona um projeto existente ao SCADA. A imagem é validada e salva antes
   * de marcar a flag, evitando que um upload inválido habilite o projeto pela
   * metade. Sem uma nova imagem, a capa existente é preservada.
   */
  async enableProject(
    projectId: string,
    ...
    if (coverImageDataUrl !== undefined) {
      const saved = await this.assets.saveDataUrl(project.tenantId, coverImageDataUrl);
  ...
  /**
   * Atualiza somente a capa de um projeto. `null` remove a imagem, enquanto
   * uma nova data URL passa pelo mesmo pipeline de validação e App Storage
   * usado na inclusão do projeto.
   */
  async updateProjectCover(
    ...
    const coverImageUrl =
      coverImageDataUrl === null
        ? null
        : (await this.assets.saveDataUrl(project.tenantId, coverImageDataUrl)).url;
  ...
  async findOne(id: string, tenantId: string | undefined, siteId?: string): Promise<ScadaScreen> {
  ...
  private async createVersion(screen: ScadaScreen): Promise<void> {
    await this.prisma.scadaScreenVersion.create({
      data: {
        screenId: screen.id,
        ...
        widgets: screen.widgets as Prisma.InputJsonValue,
        settings: screen.settings as Prisma.InputJsonValue,
      },
    });
  }
  ...
  async create(dto: CreateScadaScreenDto): Promise<ScadaScreen> {
    ...
    // Valida que o projeto/site informados pertencem ao tenant e ao mesmo site.
    const project = await this.prisma.project.findFirst({
    ...
    if (project.siteId !== dto.siteId) {
      throw new BadRequestException('O gateway não pertence ao site informado');
    }
  ...
  async update(
    ...
    if (dto.siteId === null || dto.projectId === null) {
      throw new BadRequestException('Uma tela SCADA não pode ficar sem site e gateway');
    }
    ...
    if (dto.siteId !== undefined || dto.projectId !== undefined) {
      if (!nextSiteId || !nextProjectId) {
        throw new BadRequestException('Uma tela SCADA precisa estar associada a site e gateway');
      }
      const project = await this.prisma.project.findFirst({
        where: { id: nextProjectId, tenantId: existing.tenantId },
        select: { id: true, siteId: true },
      });
      if (!project) {
        throw new NotFoundException(`Gateway ${nextProjectId} não encontrado para este cliente`);
      }
      if (project.siteId !== nextSiteId) {
        throw new BadRequestException('O gateway não pertence ao site informado');
      }
      const site = await this.prisma.site.findFirst({
        where: { id: nextSiteId, tenantId: existing.tenantId },
        select: { id: true },
      });
      if (!site) {
        throw new NotFoundException(`Site ${nextSiteId} não encontrado para este cliente`);
      }
    }
  ...
  async findVersions(id: string, tenantId: string | undefined) {
    await this.findOne(id, tenantId);
    return this.prisma.scadaScreenVersion.findMany({
      where: { screenId: id, ...(tenantId ? { tenantId } : {}) },
      orderBy: [{ createdAt: 'desc' }, { id: 'desc' }],
      select: { id: true, name: true, description: true, width: true, height: true, status: true, createdAt: true },
    });
  }

  async restoreVersion(id: string, versionId: string, tenantId: string | undefined): Promise<ScadaScreen> {
    const current = await this.findOne(id, tenantId);
    const version = await this.prisma.scadaScreenVersion.findFirst({
      where: { id: versionId, screenId: id, ...(tenantId ? { tenantId } : {}) },
    });
    if (!version) throw new NotFoundException('Versão da tela não encontrada');
    await this.createVersion(current);
    const restored = await this.prisma.scadaScreen.update({
      where: { id },
      data: {
        name: version.name,
        ...
        widgets: version.widgets as Prisma.InputJsonValue,
        settings: version.settings as Prisma.InputJsonValue,
      },
    });
    await this.unpinConflictingNavWidgets(restored);
    return restored;
  }
