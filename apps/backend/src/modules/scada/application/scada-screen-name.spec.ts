...
function makeService(siblings: Array<{ name: string }>) {
  ...
  const assets = { saveDataUrl: jest.fn() };
  const service = new ScadaService(prisma as never, assets as never);
...
describe('ScadaService — unicidade de nome das telas', () => {
  it('rejeita criação com nome duplicado no mesmo projeto (case/trim-insensitive)', async () => {
    ...
    await expect(
      service.create({ name: 'iluminação', tenantId: 'tenant-a', siteId: 'site-1', projectId: 'proj-1' } as never),
    ).rejects.toThrow(ConflictException);
  });

  it('permite criar quando não há tela com o mesmo nome no escopo', async () => {
    const { service, prisma } = makeService([{ name: 'Chillers' }]);
    await service.create({ name: 'Iluminação', tenantId: 'tenant-a', siteId: 'site-1', projectId: 'proj-1' } as never);
  ...
  it('mesmo nome em projetos diferentes é permitido (query filtra pelo projeto)', async () => {
    ...
    await service.create({ name: 'Iluminação', tenantId: 'tenant-a', siteId: 'site-1', projectId: 'proj-2' } as never);
  ...
  it('rejeita criação sem gateway, mesmo que o site seja informado', async () => {
  ...
  it('rejeita criação quando gateway e site não pertencem ao mesmo vínculo', async () => {
