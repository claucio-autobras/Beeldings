...
export class ScadaAssetService {
  ...
  async saveDataUrl(tenantId: string | undefined, dataUrl: string): Promise<SavedAsset> {
    ...
    return this.saveBuffer(tenantId, buffer, mime, ext);
  }

  /** Persiste bytes já decodificados, usado ao importar um arquivo visual. */
  async saveBuffer(
    ...
    if (buffer.byteLength === 0) throw new BadRequestException('Imagem vazia');
    if (buffer.byteLength > MAX_BYTES) {
      throw new BadRequestException('Imagem excede o limite de 5 MB');
    }
    const mime = contentType.toLowerCase().split(';', 1)[0];
    const ext = extension ?? MIME_EXT[mime];
    if (!ext) throw new BadRequestException(`Tipo de imagem não suportado: ${mime}`);
