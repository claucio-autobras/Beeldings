...
export class ScadaObjectStorageService {
  ...
  /** Lê um asset inteiro para compor um arquivo visual independente. */
  async read(relativePath: string): Promise<{ buffer: Buffer; contentType: string }> {
    const resolved = await this.getForRead(relativePath);
    const [buffer] = await resolved.file.download();
    return { buffer, contentType: resolved.contentType };
  }
}
