...
describe('EmqxProvisioningService — retained cleanup', () => {
  ...
  it('identifica uma sessão MQTT ativa e não expõe segredo', async () => {
    fetchMock
      .mockResolvedValueOnce({
        ...
        json: async () => ({
          data: [{
            ...
            connected_at: '2026-09-04T13:00:00.000Z',
          }],
        }),
      });

    await expect(service.getMqttClientConnection('dev-test')).resolves.toEqual({
      available: true,
      connected: true,
      clientId: 'plug001016',
      ipAddress: '10.0.0.16',
      connectedAt: '2026-09-04T13:00:00.000Z',
      reason: 'connected',
    });
    expect(fetchMock).toHaveBeenCalledWith(
      `${API}/clients?username=dev-test&limit=100`,
      expect.objectContaining({ headers: expect.any(Object) }),
    );
  });

  it('diferencia credencial existente sem sessão de usuário ausente', async () => {
    fetchMock
      .mockResolvedValueOnce({
        ok: true,
        status: 200,
    ...
    await expect(service.getMqttClientConnection('dev-offline')).resolves.toMatchObject({
      available: true,
      connected: false,
    ...
    await expect(service.getMqttClientConnection('dev-missing')).resolves.toMatchObject({
      available: true,
      connected: false,
