...
export interface MqttClientConnectionStatus {
  available: boolean;
  connected: boolean;
  clientId: string | null;
  ipAddress: string | null;
  connectedAt: string | null;
  reason: 'connected' | 'not_connected' | 'user_not_found' | 'api_unavailable' | 'api_error';
}
...
export class EmqxProvisioningService {
  ...
  /**
   * Consulta a sessão MQTT atual de um usuário no EMQX.
   *
   * Isso é deliberadamente separado de mqttUserExists: uma credencial pode
   * existir no banco de autenticação e ainda não haver qualquer equipamento
   * conectado. A tela de cadastro precisa distinguir esses estados.
   */
  async getMqttClientConnection(username: string): Promise<MqttClientConnectionStatus> {
    const unavailable = (
      reason: MqttClientConnectionStatus['reason'],
    ...
    if (!this.apiUrl) return unavailable('api_unavailable');

    try {
      const url =
        `${this.apiUrl}/clients?username=${encodeURIComponent(username)}&limit=100`;
      ...
      if (!res.ok) {
        this.logger.warn(`EMQX: falha ao consultar sessão do usuário ${username}: HTTP ${res.status}`);
        return unavailable('api_error');
      }

      const payload = await res.json() as { data?: unknown };
      const clients = Array.isArray(payload.data)
        ? payload.data.filter((client): client is Record<string, unknown> => (
      ...
      if (!client) {
        // A filtered /clients response usually contains only active sessions,
        // so an empty list alone cannot tell "user absent" from "user offline".
        // Use the authentication endpoint to make that distinction explicit.
        const userExists = await this.mqttUserExists(username);
        if (userExists === null) return unavailable('api_error');
        return {
          available: true,
          connected: false,
          clientId: null,
          ipAddress: null,
          connectedAt: null,
          reason: userExists ? 'not_connected' : 'user_not_found',
        };
      }
      ...
      return {
        available: true,
        connected: true,
        clientId: stringValue(client.clientid) ?? stringValue(client.clientId),
        ipAddress: stringValue(client.ip_address) ?? stringValue(client.ipAddress),
        connectedAt: stringValue(client.connected_at) ?? stringValue(client.connectedAt),
        reason: 'connected',
      };
    } catch (err) {
      this.logger.warn(`EMQX: erro ao consultar sessão do usuário ${username}: ${(err as Error).message}`);
      return unavailable('api_error');
    }
  }
  ...
  private async upsertUserAclRules(
    ...
    if (res.status === 409) {
      ...
      if (!putRes.ok) {
        ...
        throw new Error(`EMQX: falha ao atualizar ACL de ${username}: HTTP ${putRes.status} — ${body}`);
      }
      return;
    }
    if (!res.ok) {
  ...
  private async createSensorAclRules(username: string, tenantId: string, gatewayId: string): Promise<void> {
    ...
    if (res.status === 409) {
      ...
      if (!putRes.ok) {
        const body = await putRes.text();
        throw new Error(
          `EMQX: falha ao atualizar ACL de sensor para ${username}: HTTP ${putRes.status} — ${body}`,
        );
      }
      return;
    }
  ...
  private async updateMqttUserPassword(userId: string, password: string): Promise<void> {
    ...
    if (!res.ok) {
      const body = await res.text();
      throw new Error(`EMQX: falha ao atualizar senha de ${userId}: HTTP ${res.status} — ${body}`);
    }
  }
