...
export class DashboardController {
  constructor(
    ...
    private readonly siteOverview: SiteOverviewService,
  ) {}
  ...
  /**
   * GET /dashboard/site-overview — consumo e horas de funcionamento a partir
   * apenas dos pontos explicitamente classificados por um perfil técnico.
   */
  @Get('site-overview')
  async getSiteOverview(
    ...
    const effectiveTenantId = resolveTenantScope(user, tenantId);
    const effectivePeriod = period === '7d' || period === '30d' ? period : '24h';
    const effectiveSiteId = effectiveTenantId ? (siteId || undefined) : undefined;
    const to = new Date();
    const from = new Date(to.getTime() - PERIOD_MS[effectivePeriod]);
    return this.siteOverview.compute({
      tenantId: effectiveTenantId,
      siteId: effectiveSiteId,
      period: effectivePeriod,
      from,
