...
/** Tela de destino ao clicar numa queda: Dispositivos (BMS/IoT), CFTV (câmera/switch/NVR) ou SCA (controladora de acesso). */
export type OfflineDropCategory = 'devices' | 'cftv' | 'sca';

/**
 * Origem de uma queda de comunicação dentro de um bucket: a que cliente e a
 * que dispositivo/gateway ela pertence, já classificada pela tela de destino
 * correta — usada pelo tooltip (quebra por origem) e pelo roteamento de
 * clique do gráfico "Alarmes e quedas no período".
 */
export interface OfflineOriginEntry {
  tenantId: string;
  tenantName: string;
  siteName: string | null;
  entityType: 'gateway' | 'device' | 'camera';
  /** deviceId (device/câmera) ou gatewayId (gateway). */
  entityId: string;
  entityName: string;
  category: OfflineDropCategory;
  /** Transições para offline dessa entidade dentro do bucket. */
  count: number;
}
...
export interface TrendBucket {
  ...
  /** Quebra por cliente/dispositivo das quedas do bucket, maior contagem primeiro. */
  offlineBreakdown: OfflineOriginEntry[];
}
  ...
  async adminTrend(input: {
    ...
    const [activations, offlineEvents] = await Promise.all([
      ...
      this.prisma.statusEvent.findMany({
        ...
        select: { entityType: true, entityId: true, tenantId: true, at: true },
      }),
    ]);

    // Identidade por trás de cada queda: device/câmera resolve nome + categoria
    // de tela (monitoredDeviceType); gateway não tem Device próprio e cai
    // sempre em "devices" (destino sensato quando não há categoria clara).
    const deviceIds = [
      ...new Set(offlineEvents.filter((e) => e.entityType !== 'gateway').map((e) => e.entityId)),
    ];
    const tenantIds = [...new Set(offlineEvents.map((e) => e.tenantId))];
    ...
    const deviceById = new Map<string, (typeof devices)[number]>(devices.map((d) => [d.id, d] as const));
    const tenantNameById = new Map<string, string>(tenants.map((t) => [t.id, t.name] as const));

    const resolveCategory = (
      entityType: string,
      monitoredDeviceType: string | null | undefined,
    ): OfflineDropCategory => {
      // Câmeras, switches e NVRs de CFTV são gravados como entityType='camera'
      // (ver AvailabilityRecorderService) — todos pertencem à tela CFTV.
      if (entityType === 'camera') return 'cftv';
      if (entityType === 'device' && monitoredDeviceType === 'ACCESS_CONTROLLER') return 'sca';
      return 'devices';
    };
    ...
    // Acumula por entidade dentro de cada bucket (Map) antes de virar array —
    // flapping da mesma entidade no mesmo bucket soma no mesmo item.
    const breakdownMaps: Map<string, OfflineOriginEntry>[] = Array.from({ length: count }, () => new Map());
    for (const e of offlineEvents) {
      const idx = idxOf(e.at.getTime());
      buckets[idx].offlineTransitions += 1;
      const device = e.entityType === 'gateway' ? undefined : deviceById.get(e.entityId);
      const entityName =
        e.entityType === 'gateway' ? `Gateway ${e.entityId}` : (device?.name ?? `#${e.entityId.slice(0, 8)}`);
      const category = resolveCategory(e.entityType, device?.monitoredDeviceType);
      const map = breakdownMaps[idx];
      ...
      if (existing) {
        ...
        map.set(e.entityId, {
          tenantId: e.tenantId,
          tenantName: tenantNameById.get(e.tenantId) ?? '—',
          siteName: device?.site?.name ?? null,
          entityType: e.entityType as 'gateway' | 'device' | 'camera',
          entityId: e.entityId,
          entityName,
          category,
          count: 1,
    ...
    for (let i = 0; i < count; i++) {
      buckets[i].offlineBreakdown = [...breakdownMaps[i].values()].sort((a, b) => b.count - a.count);
    }
