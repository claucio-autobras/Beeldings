...
export interface AlarmEventFilters {
  ...
  from?: Date;
  to?: Date;
  /** Limita a quantidade retornada depois da ordenação por severidade/atividade. */
  limit?: number;
}
...
export class AlarmEventsService {
  ...
  async findAll(filters: AlarmEventFilters) {
    ...
    const limited = filters.limit && filters.limit > 0
      ? events.slice(0, Math.min(filters.limit, 100))
      : events;
    return limited.map((e) => this.toDto(e, ackNames));
  }
