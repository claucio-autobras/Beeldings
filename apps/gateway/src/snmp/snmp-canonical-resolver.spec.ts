...
describe('resolveCanonicalMetrics', () => {
  ...
  it('6c. memory_available — UCD kB is converted to bytes and zero is preserved', () => {
    const metrics = resolveCanonicalMetrics(true, makeOidResults([[UCD_MEM_AVAIL_OID, 0]]), EMPTY_WALK);
    expect(metrics['memory_available'].value).toBe(0);
    expect(metrics['memory_available'].selectedOid).toBe(UCD_MEM_AVAIL_OID);
    expect(metrics['memory_available'].unit).toBe('bytes');
  });
