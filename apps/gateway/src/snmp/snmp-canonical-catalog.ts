...
export const CANONICAL_METRIC_CATALOG: CanonicalMetricEntry[] = [
  ...
  // (ping/gateway ou binding legado) podem preenchê-la; não inventamos OID.
  {
    ...
    label: 'Pacotes descartados',
    unit: 'pkts',
    candidates: [
      // IF-MIB discards are the legacy SCA packet-loss counter. Keep the
      // canonical key separate from ping_loss and expose the raw counter;
      // polling applies its rate/aggregation policy later.
      {
        source: 'get',
        oid: `${IF_IN_DISCARDS_PREFIX}.1`,
        label: 'IF-MIB ifInDiscards.1',
  ...
  // ── Memória disponível ──────────────────────────────────────────────────────
  {
    canonicalKey: 'memory_available',
    label: 'Memória disponível (bytes)',
    unit: 'bytes',
    candidates: [
      // UCD memAvailReal is reported in kB. This is the recoverable-memory
      // contract used by the SCA cards; it is intentionally distinct from
      // memory_used_percent.
      { source: 'get', oid: UCD_MEM_AVAIL_OID, transform: 'kb_to_bytes', label: 'UCD memAvailReal (kB→bytes)' },
    ],
    aggregation: 'first',
  },
