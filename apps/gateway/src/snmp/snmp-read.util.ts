...
/**
 * Máximo de tentativas por OID dentro do fallback individual. Um atraso
 * pontual do firmware em UM OID (silêncio/timeout) não pode ser confundido
 * com ausência de dado — absorve com uma tentativa extra antes de desistir
 * naquele ciclo. Um erro de agente (OID rejeitado explicitamente) NUNCA é
 * retentado aqui: é uma resposta conclusiva, tentar de novo não muda o
 * resultado — conta, em vez disso, para a detecção de OID persistentemente
 * inválido (ver `readSnmpOids`/`rejectedOids`).
 */
const FALLBACK_OID_MAX_ATTEMPTS = 2;

/** Resultado bruto de uma tentativa de GET de um único OID. */
interface OidReadAttempt {
  value: number | null;
  /**
   * 'ok'          → leitura válida;
   * 'agent_error' → o agente respondeu recusando o OID (noSuchName/
   *                 noSuchObject/RequestFailedError) — rejeição conclusiva;
   * 'silence'     → nenhuma resposta (timeout/rede) — pode ser passageiro.
   */
  classification: 'ok' | 'agent_error' | 'silence';
}

function readOneOidAttempt(target: SnmpTarget, oid: string): Promise<OidReadAttempt> {
  return new Promise((resolve) => {
    const session = createSession(target);
    let settled = false;
    const done = (result: OidReadAttempt) => {
      if (settled) return;
      settled = true;
      try {
        session.close();
    ...
    session.get([oid], (error: Error | null, varbinds: snmp.VarBind[]) => {
      if (error) {
        done({ value: null, classification: classifySnmpError(error) });
        return;
      }
      const vb = varbinds[0];
      if (vb && !snmp.isVarbindError(vb)) {
        done({ value: parseSnmpNumber(vb.value), classification: 'ok' });
        return;
      }
      // Varbind com erro (noSuchObject/noSuchInstance): o agente respondeu —
      // rejeição conclusiva deste OID específico, equivalente a agent_error.
      done({ value: null, classification: 'agent_error' });
    });
    session.on('error', () => done({ value: null, classification: 'silence' }));
  });
}

/**
 * GET de UM OID numa sessão efêmera própria, com tolerância a UMA falha de
 * silêncio passageira (retry). Retorna o valor numérico, a classificação do
 * resultado final e se houve rejeição de agente em QUALQUER tentativa
 * (usado pela detecção de OID persistentemente inválido).
 * Usado no fallback por OID — a vivacidade do host já foi provada pelo lote.
 */
async function readOneOid(target: SnmpTarget, oid: string): Promise<OidReadAttempt> {
  let attempt = await readOneOidAttempt(target, oid);
  let tries = 1;
  // Só retenta silêncio: um erro de agente é definitivo, retry não ajudaria.
  while (attempt.classification === 'silence' && tries < FALLBACK_OID_MAX_ATTEMPTS) {
    await sleep(FALLBACK_GET_INTERVAL_MS);
    attempt = await readOneOidAttempt(target, oid);
    tries++;
  }
  return attempt;
}
 ...
 *
 * `rejectedOids`, quando informado, recebe cada OID cuja leitura final (após
 * as tentativas de retry) foi uma rejeição conclusiva do agente — permite ao
 * chamador (SnmpDriver) detectar um OID persistentemente inválido ao longo de
 * vários ciclos e parar de incluí-lo no lote (evitando cair neste caminho
 * lento todo ciclo por causa de um único OID morto).
 */
async function readOidsIndividually(
  ...
  for (let i = 0; i < oids.length; i++) {
    ...
    const result = await readOneOid(target, oids[i]);
    values.push(result.value);
    if (result.classification === 'agent_error') {
      rejectedOids?.add(oids[i]);
    }
  }
  return values;
}
 ...
 *
 * `rejectedOids`, quando informado, recebe cada OID que teve uma rejeição
 * conclusiva do agente nesta chamada (varbind de erro no lote, ou rejeição
 * confirmada no fallback individual). Não é preenchido para timeouts/silêncio
 * — só uma resposta explícita do agente conta como rejeição. O chamador
 * (SnmpDriver) usa isso para detectar, ao longo de vários ciclos, um OID
 * persistentemente inválido e parar de incluí-lo no lote.
 */
export function readSnmpOids(
  ...
  return new Promise((resolve) => {
    ...
    session.get(queryOids, (error: Error | null, varbinds: snmp.VarBind[]) => {
      if (error) {
        ...
        if (classifySnmpError(error) === 'agent_error') {
          if (oids.length > 1) {
            ...
            settled = true;
            void readOidsIndividually(target, oids, rejectedOids).then(resolve);
            return;
          }
          if (oids.length > 0) rejectedOids?.add(oids[0]);
      ...
      const values = varbinds.map((vb, i) => {
        if (snmp.isVarbindError(vb)) {
          if (i < oids.length) rejectedOids?.add(oids[i]);
          return null;
        }
