...
describe('walk genérico — paridade com snmpwalk (iDFlex como referência)', () => {
  ...
  it('perfil Control iD corrigido: CPU % proprietária não substitui a fonte canônica', async () => {
    // O firmware iDFlex validado tem OIDs de temperatura publicados durante
    // descoberta em algumas imagens, mas não os expõe como fonte comprovada
    // de temperatura em produção. O perfil deve mantê-la explicitamente
    // indisponível e usar hrProcessorLoad como CPU canônica.
    const byMetric = new Map(
      CONTROL_ID_PROFILE.mappings.map((m) => [m.metricKey, m]),
    );
    const cpuMap = byMetric.get('cpu')!;
    const tempMap = byMetric.get('temperature')!;
    expect(cpuMap.tableOidPrefix).toBe('1.3.6.1.2.1.25.3.3.1.2');
    expect(tempMap.oid).toBe('');
  }, 15_000);
});
