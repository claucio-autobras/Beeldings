...
- O remoto `github.com/claucio-autobras/Beeldings` é sincronizado nos DOIS sentidos somente sob solicitação explícita, por `scripts/sync-github.sh` (`push` / `push-snapshot` / `pull` / `status`; precisa de `GITHUB_TOKEN`). Não há daemon no workflow `Project` nem sincronização no `post-merge`; iniciar a workspace e fazer merge não publica nada no GitHub. Um lock compartilhado, a proteção contra divergência e a recusa de force-push continuam valendo (`init-base` é a exceção destrutiva para primeiro setup/recuperação). O histórico completo local (~3,8 GB) nunca é enviado. Guia completo e sequência manual: `docs/github-sync.md`; procedimento canônico: `.agents/memory/bluebee-github-sync.md`.
