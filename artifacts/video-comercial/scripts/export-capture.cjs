...
const TOTAL_MS = parseInt(process.env.TOTAL_MS || "113500");
const PROFILE = '/tmp/vidcap/profile';

(async () => {
  fs.mkdirSync(`${PROFILE}/Default`, { recursive: true });
  fs.writeFileSync(`${PROFILE}/Default/Preferences`, JSON.stringify({
  ...
  try {
    require('child_process').execSync(`${process.env.XDOTOOL} key Escape`, { env: process.env });
    require('child_process').execSync(`${process.env.XDOTOOL} mousemove 1775 50 click 1`, { env: process.env });
    require('child_process').execSync(`${process.env.XDOTOOL} mousemove 1919 1079`, { env: process.env });
  } catch (e) {
    console.log('browser overlay cleanup failed', e.message);
  }
