...
cd "$WORKDIR"
NODE_PATH="$WORKDIR/node_modules" URL="http://localhost:$PORT/" TOTAL_MS="$TOTAL_MS" OUT="$WORKDIR/det.mp4" \
  node "$ART_DIR/scripts/export-deterministic.cjs"
