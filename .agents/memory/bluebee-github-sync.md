...
attached local `main`; each idle cycle must still inspect the remote head so IDE changes are
detected promptly. Excluded media-only changes can advance the local sync point without
