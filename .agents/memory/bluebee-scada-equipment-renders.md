...
**Rules:**
- Built-in equipment PNGs (HVAC/BMS set + a 14-render fire set under a separate "Incêndio" palette category) live in `apps/frontend/public/scada-equipment/<type>.png` (transparent bg, generated via media-generation with a shared "digital-twin, light gray/white, isometric" prompt style + removeBackground). Regenerate with the same style string to keep the set consistent.
...
- New equipment types must be added to EVERY type registry/set in the SCADA module in lockstep (types, asset/aspect maps, renderer + properties allowlists, defaults, palette, layers labels) — grep an existing type like 'smoke-detector' to find them all; missing one silently breaks binding or rendering.
- Fire-set default state rules invert the convention: 1 = alarm (red pulse), 0 = normal (green) — including smoke-detector; only fire-pump behaves like other pumps (1 = on, green pulse). Defaults apply on creation only; saved widgets keep stored rules.
