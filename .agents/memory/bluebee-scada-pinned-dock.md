...
Navigation bar alignment values are semantic (`left`/`center`/`right`, `top`/`center`/`bottom`); map them to `flex-start`/`center`/`flex-end` before assigning `justify-content`, especially for the vertical sidebar setting.
