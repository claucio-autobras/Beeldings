...
- Estilo de fluxo é exclusivo: `dash` mostra só tracejado, `arrow` só setas e `water` só faixa contínua; `showDirectionArrow` permanece apenas como campo legado ignorado pelo renderer.
- **Why:** tratar a seta como sobreposição fazia o editor salvar combinações visuais conflitantes e tornava dash/água ambíguos; a escolha de Estilo precisa ser a única fonte de verdade.
- **How to apply:** ao adicionar estilos ou ajustar telas antigas, preserve `dash`/`water`, use `arrow` para direção e mantenha o estado parado sem camadas de movimento.
