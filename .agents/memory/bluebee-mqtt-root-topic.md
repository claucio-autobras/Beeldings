...
- Provisionamento da credencial dedicada do device é pré-requisito do cadastro: falha de usuário/ACL deve abortar e limpar o device recém-criado, nunca retornar credencial parcial; ACL nega tudo fora do raiz (deny `#` por último).
- **Credencial raiz é vinculada ao ID do Device (`dev-<id>`):** apagar e recriar o mesmo Aeris muda usuário/senha, então o equipamento físico precisa receber a nova credencial; o trace antigo não valida o cadastro atual.
  **Why:** o Aeris `008065` conectava com um cadastro raiz antigo, enquanto recriações posteriores geraram IDs e credenciais diferentes.
  **How to apply:** antes de testar, use sempre a credencial exibida na edição do único cadastro vigente; não crie outro cadastro para “tentar de novo”.
